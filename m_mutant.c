...
qboolean mutant_check_jump(edict_t *self)
{
	...
	{
		if (self->absmin[2] > (self->enemy->absmin[2] + 0.75 * self->enemy->size[2]) + self->monsterinfo.jumpdn )
			return false;

		if (self->absmax[2] < (self->enemy->absmin[2] + 0.25 * self->enemy->size[2]) - self->monsterinfo.jumpup )
			return false;
	...
	v[2] = 0;
	const float distance = VectorLength(v);
...
qboolean mutant_checkattack(edict_t *self)
{
	...
	if (mutant_check_jump(self))
	{
		...
		return true; // FIXME play a jump sound here
	}
...
void SP_monster_mutant (edict_t *self)
{
	...
	if (!self->monsterinfo.flies)
		self->monsterinfo.flies = 0.9f;

	if (self->health < 0)
	{
		mmove_t	*deathmoves[] = { &mutant_move_death1, &mutant_move_death2, NULL };
