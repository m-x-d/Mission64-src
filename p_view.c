...
void P_DamageFeedback(edict_t *player)
{
	...
	count = max(10, count); // always make a visible effect

	// play an apropriate pain sound
	if (level.time > player->pain_debounce_time && !(player->flags & FL_GODMODE) && client->invincible_framenum <= level.framenum)
	{
		int l;
		const int r = 1 + (rand() & 1);
	...
	client->damage_alpha = max(0, client->damage_alpha);
	client->damage_alpha += count * 0.01f;
	client->damage_alpha = clamp(client->damage_alpha, 0.2, 0.6); // don't go too saturated
	...
	if (kick && player->health > 0)	// kick of 0 means no view adjust at all
	{
		...
		const float halfcount = count * 0.5; //mxd
		kick = clamp(kick, halfcount, 50);
...
//===================================
	...
	else
	{
		...
		float kickratio = (ent->client->v_dmg_time - level.time) / DAMAGE_TIME;
		...
		angles[PITCH] += kickratio * ent->client->v_dmg_pitch;
		angles[ROLL] += kickratio * ent->client->v_dmg_roll;

		// add pitch based on fall kick
		kickratio = (ent->client->fall_time - level.time) / FALL_TIME;
		kickratio = max(0, kickratio);
		angles[PITCH] += kickratio * ent->client->fall_value;

		// add angles based on velocity
		float delta = DotProduct(ent->velocity, forward);
...
//===================================
	...
	// add fall height
	float fallratio = (ent->client->fall_time - level.time) / FALL_TIME;
	fallratio = max(0, fallratio);
	v[2] -= fallratio * ent->client->fall_value * 0.4;
...
//	gi.DebugGraph (bob *2, 255);
	...
	if (ent->client->chasetoggle)
	{
		...
		if (ent->client->chasecam != NULL)
			for (int i = 0; i < 3; i++)
				ent->client->ps.pmove.origin[i] = ent->client->chasecam->s.origin[i] * 8;
	}
...
void SV_AddBlend(float r, float g, float b, float a, float *v_blend)
{
	...
	const float a2 = v_blend[3] + (1 - v_blend[3]) * a; // new total alpha
	const float a3 = v_blend[3] / a2; // fraction of color from old
...
void SV_CalcBlend(edict_t *ent)
{
	...
	const int contents = gi.pointcontents(vieworg);
	...
	ent->client->damage_alpha = max(0, ent->client->damage_alpha - 0.06);

	// drop the bonus value
	ent->client->bonus_alpha = max(0, ent->client->bonus_alpha - 0.1);
...
void P_SlamDamage(edict_t *ent)
{
	...
	deltav[2] = 0;
	float delta = VectorLength(deltav);
...
//ZOID
	...
	if (delta > 40 * (player_max_speed->value / 300)) // Knightmare changed
	{
		...
		int damage = (delta - 40 * (player_max_speed->value / 300)) / 2; // Knightmare changed
		damage = max(1, damage);
...
//ZOID
	...
	// Lazarus: Changed here to NOT play footstep sounds if ent isn't on the ground, so player will no longer play footstep sounds when descending a ladder.
...
void P_WorldEffects(void)
{
	...
	const int old_watertype = current_player->old_watertype;
	current_client->old_waterlevel = waterlevel;
	current_player->old_watertype  = current_player->watertype;

	const qboolean breather = current_client->breather_framenum > level.framenum;
	const qboolean envirosuit = current_client->enviro_framenum > level.framenum;
	...
	if (waterlevel && (current_player->watertype & (CONTENTS_LAVA | CONTENTS_SLIME)))
	{
		if (current_player->watertype & CONTENTS_LAVA)
		{
			...
			const int damage = (envirosuit ? waterlevel : 3 * waterlevel); // take 1/3 damage with envirosuit
			T_Damage(current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, damage, 0, 0, MOD_LAVA);
		}

		if (!envirosuit && current_player->watertype & CONTENTS_SLIME) //TODO: mxd. can the player be both in lava and slime at the same time?..
		{
			// no damage from slime with envirosuit
			T_Damage(current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, waterlevel, 0, 0, MOD_SLIME);
		}
	}
}
...
void G_SetClientEffects(edict_t *ent)
{
	...
	{
		const int pa_type = PowerArmorType(ent);
...
//ZOID
	...
	if (ent->client->flashlight)
	{
		...
		{
			const int flashlightindex = ITEM_INDEX(FindItem(FLASHLIGHT_ITEM)); //mxd
			if (!Q_stricmp(FLASHLIGHT_ITEM,"health") || ent->client->pers.inventory[flashlightindex] >= level.flashlight_cost)
			{
				// Player has items remaining
				if (ent->client->flashlight_time <= level.time)
				{
					ent->client->pers.inventory[flashlightindex] -= level.flashlight_cost;
		...
		if (ent->client->flashlight)
		{
			...
			const trace_t tr = gi.trace(start, NULL, NULL, end, ent, CONTENTS_SOLID | CONTENTS_MONSTER | CONTENTS_DEADMONSTER);
...
void G_SetClientEvent(edict_t *ent)
{
	...
	else if (level.framenum % 4 == 0)
	{
		if (!ent->waterlevel && ent->movetype != MOVETYPE_NOCLIP && fabs(ent->velocity[2]) > 50)
		{
			...
			const trace_t tr = gi.trace(ent->s.origin, ent->mins, ent->maxs, end, ent, CONTENTS_LADDER);
			if (tr.fraction < 1.0)
...
void G_SetClientSound(edict_t *ent)
{
	...
	const char *weap = (ent->client->pers.weapon ? ent->client->pers.weapon->classname : "");
...
qboolean PlayerOnFloor(edict_t *player)
{
	...
	const trace_t tr = gi.trace(player->s.origin, NULL, NULL, end, player, MASK_ALL);

	if (tr.fraction >= sv_step_fraction->value)
		return false;

	if (player->client->oldvelocity[2] > 0 || player->velocity[2] > 0)
		return false;
...
void G_SetClientFrame(edict_t *ent)
{
	...
	const qboolean duck = (client->ps.pmove.pm_flags & PMF_DUCKED);

	// Knightmare- don't always play running animation when in mud
	qboolean run = (ent->in_mud && xyspeed > 40) || (!ent->in_mud && xyspeed);
	...
	const qboolean floor = PlayerOnFloor(ent);
...
newanim:
	...
	if (!ent->groundentity && (!floor || ent->waterlevel > 2))
	{
...
void ClientEndServerFrame(edict_t *ent)
{
	...
	// 
	// If it wasn't updated here, the view position would lag a frame behind the body position when pushed -- "sinking into plats"
	//
	for (int i = 0; i < 3; i++)
	{
	...
	//
	// set model angles from view angles so other things in the world can tell which direction you are looking
	//
	...
	float bobtime = current_client->bobtime;
