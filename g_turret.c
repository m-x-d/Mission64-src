...
edict_t	*TurretTarget (edict_t *self)
{
	...
	for (int i = 1; i < globals.num_edicts; i++)
	{
		...
		if (!who->inuse || who->takedamage == DAMAGE_NO || who->solid == SOLID_NOT)
			continue;

		VectorMA(who->absmin, 0.5f, who->size, end);
...
//CW--
	...
	const int speed = 550 + 50 * skill->value;
	...
	if (self->delay < level.time)
	{
		switch (self->sounds)
		{
			case 1: // railgun
			{
				...
				if (self->moreflags & FL_TURRET_DOUBLE) //CW++
				{
					if (self->moreflags & FL_TURRET_DOUBLE_ALT)
					{
						if (self->moreflags & FL_TURRET_DOUBLE_ALT_FIRING)
			...
			case 4: // Homing rockets
			{
				...
				else if (self->spawnflags & SF_TURRET_PLAYER_CONTROLLABLE || allow_player_use_abandoned_turret->value)
				{
					...
					edict_t *target = TurretTarget(self);
			...
			{
				HB_Shots++;
				const int effect = (!(HB_Shots % 4) ? EF_HYPERBLASTER : 0);
				if (self->moreflags & FL_TURRET_DOUBLE) //CW++
				{
					if (self->moreflags & FL_TURRET_DOUBLE_ALT)
					{
						if (self->moreflags & FL_TURRET_DOUBLE_ALT_FIRING)
						{
							fire_blaster(owner, start2, forward2, self->wait, 1000, effect, true, BLASTER_ORANGE);
							self->moreflags &= ~FL_TURRET_DOUBLE_ALT_FIRING;
						}
						else
						{
							fire_blaster(owner, start, forward, self->wait, 1000, effect, true, BLASTER_ORANGE);
							self->moreflags |= FL_TURRET_DOUBLE_ALT_FIRING;
						}
					}
					else
...
void turret_disengage (edict_t *self)
{
	...
	forward[2] = max(80, forward[2] + 150);
...
void turret_turn (edict_t *self)
{
	...
	if (self->viewer && self->viewer->client)
	{
		...
		if (client->old_owner_angles[0] != client->ucmd.angles[0] || client->old_owner_angles[1] != client->ucmd.angles[1])
		{
			...
			const float delta_time = level.time - self->touch_debounce_time;
			if (delta_time < 0 || delta_time > 1.0)
			{
				const float fastest = self->speed * FRAMETIME;
				float delta_angle = SHORT2ANGLE(client->ucmd.angles[0] - client->old_owner_angles[0]);
...
//=======	
	...
	self->move_angles[PITCH] = clamp(self->move_angles[PITCH], self->pos2[PITCH], self->pos1[PITCH]);

	// Lazarus: Special case - if there are no constraints on YAW, don't adjust angle
	const qboolean yaw_restrict = (self->pos1[YAW] != 0 || self->pos2[YAW] != 360);

	if (yaw_restrict)
	{
		float yaw_range = self->pos2[YAW] - self->pos1[YAW];
		if (yaw_range < 0)
			yaw_range += 360;

		float yaw_base = self->move_angles[YAW] - self->pos1[YAW];
		if (yaw_base < 0)
			yaw_base += 360;

		if (yaw_base > yaw_range)
		{
			float dmin = fabsf(self->pos1[YAW] - self->move_angles[YAW]);
			if (dmin < 0)
				dmin += 360;
			else if (dmin > 360)
				dmin -= 360;

			float dmax = fabsf(self->pos2[YAW] - self->move_angles[YAW]);
			if (dmax < 0)
				dmax += 360;
			else if (dmax > 360)
				dmax -= 360;
	...
	delta[0] = clamp(delta[0], -self->speed, self->speed);
	delta[1] = clamp(delta[1], -self->speed, self->speed);
...
void turret_breach_think(edict_t *self)
{
	...
	turret_turn(self);
	float yaw_r = self->pos2[YAW] - self->pos1[YAW];
	if (yaw_r < 0)
		yaw_r += 360;
	...
	const qboolean remote_monster = (self->owner && self->owner->target_ent == self && (self->owner->spawnflags & SF_TURRETDRIVER_REMOTE_DRIVER));
	...
	if (self->owner && !remote_monster)
	{
		if (self->owner->target_ent == self)
		{
			...
			// z
			angle = self->s.angles[PITCH] * (M_PI2 / 360);
			const float target_z = SnapToEights(self->s.origin[2] + self->owner->move_origin[0] * tanf(angle) + self->owner->move_origin[2]);

			const float diff = target_z - self->owner->s.origin[2];
		...
		if ((self->spawnflags & SF_TURRET_PLAYER_CONTROLLABLE) || allow_player_use_abandoned_turret->value)
		{	
	...
	{	
		...
		ent++;
		for (int i = 0; i < maxclients->value; i++, ent++)
		{
			...
			{	
				...
				//ed - set the flag on the client so that when they shoot the turret shoots instead of "using" it
				ent->flags |= FL_TURRET_OWNER;
			}
		}
	}
	...
	if ((self->spawnflags & SF_TURRET_TRACKING) && (!self->owner || remote_monster))
	{
		...
		if (self->enemy)
		{
			// check that current enemy is valid. if so, find distance.
			// don't switch enemies unless another monster is at least 100 units closer to the camera
			if (self->enemy->inuse)
			{
				if (self->enemy->health > self->enemy->gib_health && !(self->enemy->svflags & SVF_NOCLIENT) && !(self->enemy->flags & FL_NOTARGET))
				{
...
good_enemy:
		if (self->enemy)
		{
			...
			if (self->sounds == 7)
			{
				...
				{
					// Can't get a grenade to target. Correct yaw but not pitch
					const vec_t pitch = self->move_angles[PITCH];
			...
			if (victim && self->sounds >= 0 && DotProduct(forward, dir) > 0.99)
			{
				...
				{
					// don't fire rockets or homing rockets if remote turret_driver is too close to target
					if (remote_monster)
					{
						vec3_t	range;
						VectorSubtract(self->enemy->s.origin, self->owner->s.origin, range);
						const vec_t r = VectorLength(range);
						if (r < 128)
							return;
					}
					...
					if (self->sounds == 5 || self->sounds == 6)
						...
						reaction_time = max(0.0, 0.5 * (2 - skill->value));
...
{
	edict_t *target = G_Find(NULL, FOFS(targetname), self->destroytarget);
	while (target)
	{
...
void turret_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if (self->deadflag != DEAD_DEAD)
	{
		...
		if (self->owner && self->owner->target_ent == self)
		{
			if (self->owner->spawnflags & SF_TURRETDRIVER_REMOTE_DRIVER)
			{
				...
				edict_t	*monster = self->owner->child;
				if (monster)
				{
		...
		if (self->deathtarget)
		{
			edict_t *target = G_Find(NULL, FOFS(targetname), self->deathtarget);
			while (target)
			{
				if (target && target->use)
	...
	self->speed /= 4; // Slow turret down and level it... or set to minpitch for MD2 turrets 
...
void use_turret_breach (edict_t *self, edict_t *other, edict_t *activator)
{
	if (self->spawnflags & SF_TURRET_TRIGGER_SPAWN)
	{
		...
		self->solid = ((self->spawnflags & SF_TURRET_MD2) ? SOLID_BBOX : SOLID_BSP);
		self->think = turret_breach_think;
		self->think(self);
	}
}
...
{
	// This added for Lazarus to help prevent player from becoming stuck when jumping onto a TRACK turret.

	// We only care about TRACK turrets. For monster controlled turrets the angles should of course be controlled by the monster.
	if (!(self->spawnflags & SF_TURRET_TRACKING))
		return;

	// We only care about players... everybody else knows better than to get tangled up with turret :-)
	if (!other->client)
		return;
	...
	{
		if (fabsf(self->s.angles[PITCH] - self->pos1[PITCH]) < fabsf(self->s.angles[PITCH] - self->pos2[PITCH]))
			self->move_angles[PITCH] = self->pos2[PITCH];
		else
			self->move_angles[PITCH] = self->pos1[PITCH];
		
		if (fabsf(self->s.angles[YAW] - self->pos1[YAW]) < fabsf(self->s.angles[YAW] - self->pos2[YAW]))
			self->move_angles[YAW] = self->pos2[YAW];
		else
			self->move_angles[YAW] = self->pos1[YAW];
	}
...
void SP_turret_breach (edict_t *self)
{
	...
	// Lazarus: Added so monsters will attack turrets that fire at them
...
void turret_base_finish(edict_t *self)
{
	if (self->team)
	{
		...
		const vec_t radius = (self->maxs[0] - self->mins[0]) * (self->maxs[0] - self->mins[0]) +
...
void turret_driver_think(edict_t *self)
{
	...
	if (self->target_ent->sounds == 7 && !AimGrenade(self->target_ent, self->target_ent->s.origin, target, self->target_ent->fog_far, dir)) // grenade launcher
	{
	...
	if (self->target_ent->sounds == 5 || self->target_ent->sounds == 6)
		...
		reaction_time = max(0.0, (2 - skill->value));
...
void turret_driver_link (edict_t *self)
{
	...
	// DWH: We no longer require turret_breach to be teamed with a turret_base UNLESS there's a turret_driver
	...
	// DWH: REMOTE (=1) drivers aren't part of the turret team, and don't think (the turret_breach thinks for them)
	if (!(self->spawnflags & SF_TURRETDRIVER_REMOTE_DRIVER))
	{
	...
	// DWH: REMOTE (=1) drivers don't move with turret, and the turret tracks players
	if (self->spawnflags & SF_TURRETDRIVER_REMOTE_DRIVER)
	{
		self->target_ent->spawnflags |= SF_TURRET_TRACKING;
	}
...
void SP_turret_driver (edict_t *self)
{
	...
	if (self->spawnflags & SF_TURRETDRIVER_REMOTE_DRIVER)
	{
		// remote turret driver - go ahead and create his "real" infantry replacement NOW so the switch won't be so time-consuming
