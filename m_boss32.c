...
void MakronHyperblaster(edict_t *self)
{
	...
	const int flash_number = MZ2_MAKRON_BLASTER_1 + (self->s.frame - FRAME_attak405);
	...
	const int frame = (self->s.frame <= FRAME_attak413 ? FRAME_attak413 : FRAME_attak421);
	dir[1] = self->s.angles[1] - 10 * (self->s.frame - frame);
	dir[2] = 0;
...
qboolean Makron_CheckAttack(edict_t *self)
{
	if (self->enemy->health > 0)
	{
		...
		const trace_t tr = gi.trace(spot1, NULL, NULL, spot2, self, CONTENTS_SOLID | CONTENTS_MONSTER | CONTENTS_SLIME | CONTENTS_LAVA);
	...
	vec3_t temp;
	const int enemy_range = range(self, self->enemy);
	VectorSubtract(self->enemy->s.origin, self->s.origin, temp);
	const float enemy_yaw = vectoyaw(temp);
...
//		self->gib_health = -2000;
	...
	if (self->health < 0)
	{
		mmove_t	*deathmoves[] = { &makron_move_death2, &makron_move_death3, NULL };
...
void MakronSpawn(edict_t *self)
{
	...
	// Jump at player
	edict_t *player = level.sight_client;
	if (!player)
		return;
