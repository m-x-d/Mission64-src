...
{
	// Lazarus. This turns off the "owner" value of a dead monster that medic previously aborted on after 2 seconds.
	// If the delay was NOT used, medic might just rotate around on top of monster continuously trying to revive him unsuccessfully.
	// And if owner isn't turned off, medic would never try to revive the monster once an abortHeal was called.
...
{
	edict_t *monster = self->activator;
	if (monster)
	{
...
// Lazarus: embedded returns true if argument entity's bounding box intersects a solid.
qboolean embedded(edict_t *ent)
{
	const trace_t tr = gi.trace(ent->s.origin, ent->mins, ent->maxs, ent->s.origin, ent, MASK_MONSTERSOLID);
...
edict_t *medic_FindDeadMonster(edict_t *self)
{
	...
	{
		if (ent == self || !(ent->svflags & SVF_MONSTER) || ent->monsterinfo.aiflags & AI_GOOD_GUY || ent->owner || ent->health > 0)
			continue;
		...
		if (!visible(self, ent) || embedded(ent) || !canReach(self, ent))
			continue;
...
void medic_StopPatrolling(edict_t *self)
{
	...
	if (!(self->monsterinfo.aiflags & AI_MEDIC) && medic_FindDeadMonster(self))
		return;
...
void medic_NextPatrolPoint(edict_t *self, edict_t *hint)
{
	...
	if (switch_paths && hint_chain_count > 1)
	{
		...
		{
			e = &g_edicts[i];
			if (!e->inuse || Q_stricmp(e->classname, "hint_path"))
				continue;

			if (next && e->hint_chain_id == next->hint_chain_id)
				continue;

			if (!visible(self, e) || !canReach(self, e))
				continue;
...
void medic_idle(edict_t *self)
{
	...
	if (self->monsterinfo.aiflags & AI_MEDIC)
		abortHeal(self, false); // Then we must have reached this point after losing sight of our patient.

	if (medic_FindDeadMonster(self))
		return;

	// If the map has hint_paths, AND the medic isn't at a HOLD point_combat, AND the medic has previously called FoundTarget(trail_time set to
	// level.time), then look for hint_path chain and follow it, hopefully to find monsters to resurrect
	if (self->monsterinfo.aiflags & AI_HINT_TEST)
		return;

	if (hint_chains_exist && !(self->monsterinfo.aiflags & AI_STAND_GROUND)	&& (self->monsterinfo.trail_time > 0 || medic_test))
	{
		edict_t	*hint=NULL;
		vec3_t	dir;
...
void medic_cable_attack(edict_t *self)
{
	...
	// See if our enemy has changed to a client, or our target has more than 0 health, abort it .. we got switched to someone else due to damage
	...
	const float distance = VectorLength(dir);
	...
	// Lazarus: Check for enemy behind muzzle... don't do these guys, 'cause usually this results in monster entanglement
	VectorNormalize(dir);
...
/*	vectoangles(dir, angles);
	...
	const trace_t tr = gi.trace(start, NULL, NULL, self->enemy->s.origin, self, MASK_SHOT);
	...
	else if (self->s.frame == FRAME_attack50)
	{
		...
		// Lazarus: reset initially dead monsters to use the INVERSE of their initial health, and force gib_health to default value
		...
		{
			// Lazarus: this should make oblivious monsters find player again
...
qboolean medic_checkattack(edict_t *self)
{
	...
	if (self->monsterinfo.aiflags & AI_MEDIC)
	{
		...
		// If we have 5 seconds or less before a timeout, look for a hint_path to the target
		if (self->timestamp < level.time + 5 && self->monsterinfo.last_hint_time + 5 < level.time)
		{
		...
		const float r = realrange(self, self->enemy);
		...
		if (r < MEDIC_MIN_DISTANCE)
		{
			abortHeal(self, false);
			return false;
		}

		// Lazarus 1.6.2.3: if point-to-point vector from cable to target is blocked by a solid
		AngleVectors(self->s.angles, forward, right, NULL);
		// Offset [8] has the largest displacement to the left... not a sure thing but this one should be the most severe test.
		VectorCopy(medic_cable_offsets[8], offset);
		G_ProjectSource(self->s.origin, offset, forward, right, start);

		const trace_t tr = gi.trace(start, NULL, NULL, self->enemy->s.origin, self, MASK_SHOT|MASK_WATER);
...
{
	if (deathmatch->value || world->effects & FX_WORLDSPAWN_CORPSEFADE)
	{
		G_FreeEdict(self);
		return;
	}
	...
	if (self->health < 0)
	{
		mmove_t *deathmoves[] = { &medic_move_death, NULL };
