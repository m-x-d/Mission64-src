...
//		ent->noise_index = gi.soundindex("misc/trigger1.wav");
		ent->noise_index = -1; // DWH - should be silent
...
{
	// make old maps work because I messed up on flag assignments here. Triggered was on bit 1 when it should have been on bit 4
	if (ent->spawnflags & 1)
	{
		vec3_t	v;
...
//mxd
void trigger_relay_autotrigger(edict_t *self)
{
...
// end DWH
	...
	{
		self->think = trigger_relay_autotrigger;
...
4 = Silent. If set, neither the "You need" message and sound nor keyuse.wav
    are played. This is useful for trigger_keys used only to remove items from the player.

Lazarus also removes non-multi-use trigger_keys once used, to free up space in the edicts array.
...
void trigger_key_use(edict_t *self, edict_t *other, edict_t *activator)
{
	...
	const int index = ITEM_INDEX(self->item);
	...
	if (coop->value)
	{
		...
		if (strcmp(self->item->classname, "key_power_cube") == 0)
		{
			...
			for (int player = 1; player <= game.maxclients; player++)
			{
				...
				// DWH: keep key
				if (!(self->spawnflags & 2) && ent->client->pers.power_cubes & (1 << cube))
				{
...
void hurt_use(edict_t *self, edict_t *other, edict_t *activator)
{
	if (self->solid == SOLID_NOT)
	{
		...
		// Lazaurs: Add check for non-moving (i.e. idle monsters) within trigger_hurt at first activation
		edict_t	*touch[MAX_EDICTS];
		const int num = gi.BoxEdicts(self->absmin, self->absmax, touch, MAX_EDICTS, AREA_SOLID);
		for (int i = 0; i < num; i++)
		{
			edict_t *hurtme = touch[i];
...
void hurt_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	{
		// DWH - Original code would fail to play a sound for SF=16 unless player just HAPPENED to hit
		//       trigger_hurt at framenum = an integral number of full seconds.
	...
	if (self->dmg > 0)
	{
		...
		if (damage > 0)
		{
			const int dflags = ((self->spawnflags & SF_HURT_NO_PROTECTION) ? DAMAGE_NO_PROTECTION : 0);
			T_Damage(other, self, self, vec3_origin, other->s.origin, vec3_origin, damage, self->dmg, dflags, MOD_TRIGGER_HURT);
		}
	}
	else
	{
		other->health = min(other->max_health, other->health - self->dmg);
	}
}
...
//		self->noise_index = gi.soundindex("misc/trigger1.wav");
		self->noise_index = -1; // DWH - should be silent
...
void trigger_inside_think(edict_t *self)
{
	...
	{
		edict_t *hit = touch[i];
		if (!hit->inuse || !hit->targetname || Q_stricmp(self->pathtarget, hit->targetname))
			continue;
...
{
	edict_t *touch[MAX_EDICTS];
	int num = gi.BoxEdicts(self->absmin, self->absmax, touch, MAX_EDICTS, AREA_SOLID);

	int weight = 0;
	for (int i = 0; i < num; i++)
	{
		edict_t *hit = touch[i];
		if (!hit->inuse || !hit->mass)
			continue;
		...
		const float f = (1.0f - fx - fy + fx*fy);
...
{
	if (self->nextthink) // already "touched" and waiting
		return;
...
void trigger_look_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (self->nextthink) // already been triggered
		return;
	...
	else
	{
		...
		// FIXME: The following is more or less accurate if the bleft-tright box is roughly a cube.
		// If it's considerably longer in one direction we'll get false misses.
...
{
	edict_t *touching = NULL;
	for (int i = 1; i <= maxclients->value && !touching; i++)
	{
...
void WriteTransitionEdict (FILE *f, edict_t *changelevel, edict_t *ent)
{
	...
	if (!Q_stricmp(e.classname, "target_laser") || !Q_stricmp(e.classname, "target_blaster"))
	...
	// wipe out all edict_t and function members, since they won't be valid in the next map and might otherwise cause.... umm... big crash
	byte *temp = (byte *)&e;
	for (field_t *field = fields; field->name; field++)
	{
		if (field->type == F_EDICT || field->type == F_FUNCTION)
		{
			auto *p = (void *)(temp + field->ofs);
			*(edict_t **)p = NULL;
		}
	}
	...
	// If the ent is a live bad guy monster, remove him from the total monster count. He'll be added back in in the new map.
	...
	// Enemy isn't preserved... let's try a new flag for single-player only that tells monster to find the player again at startup
	if (!coop->value && !deathmatch->value && ent->enemy == &g_edicts[1] && ent->health > 0)
