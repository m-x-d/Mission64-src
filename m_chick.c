...
{
	if (!(self->monsterinfo.aiflags & AI_STAND_GROUND) && random() <= 0.3)
...
void chick_pain(edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (skill->value > 1 || damage <= 10) //CW: no pain anims in hard, shrug off low damage
		return;
...
{
	const qboolean do_misfire = (random() < 0.07f);
	const qboolean do_click = (do_misfire || random() > 0.4f);
	...
	if (do_misfire)
	{
		...
		const int rocketSpeed = 200; //500 + (100 * skill->value);
...
{
	// DWH: Added skill level-dependent rocket speed, leading target, suicide prevention, target elevation dependent target location, and homing rockets
	...
	if (visible(self, self->enemy))
	{
		...
		// DWH: Switched this around from Rogue code... it led target more often for Easy, which seemed backwards
		if (random() < (0.2f + skill->value * 0.15f) && !(self->spawnflags & SF_MONSTER_SPECIAL))
		{
	...
	if (trace.ent == self->enemy || trace.ent == world)
	{
		...
		if (VectorLength(vec) > MELEE_DISTANCE && (trace.fraction > 0.5f || (trace.ent && trace.ent->client)))
...
void chick_skip_frames(edict_t *self)
{
	if (skill->value >= 1)
	{
		...
		if (skill->value > 1 && self->s.frame == FRAME_attak109)
...
void chick_rerocket(edict_t *self)
{
	if (self->enemy->health > 0 && range(self, self->enemy) > RANGE_MELEE && visible(self, self->enemy) && random() <= 0.6)
...
void chick_reslash(edict_t *self)
{
	if (self->enemy->health > 0 && range(self, self->enemy) == RANGE_MELEE && random() <= 0.9)
...
void SP_monster_chick(edict_t *self)
{
	...
	if (self->health < 0)
	{
		mmove_t	*deathmoves[] = { &chick_move_death1, &chick_move_death2, NULL };
		M_SetDeath(self, (mmove_t **)&deathmoves);
	}

	self->common_name = "Ion Maiden";
