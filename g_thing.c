...
{
	edict_t *monster = self->target_ent;
	if (level.time <= self->touch_debounce_time && monster && monster->inuse && monster->movetarget == self && monster->health > 0)
	{
		vec3_t dir;
		VectorSubtract(monster->s.origin, self->s.origin, dir);
		dir[2] = 0;
...
void thing_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	{
		// For monster/actor seeking cover after firing, pause a random bit before resuming attack
		self->touch_debounce_time = level.time + random() * 6.0f;
		edict_t *monster = self->target_ent;
		monster->monsterinfo.stand(monster);
