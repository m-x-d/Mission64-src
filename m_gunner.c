...
{
	if (!(self->monsterinfo.aiflags & AI_STAND_GROUND) && random() <= 0.05)
...
void gunner_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (skill->value == 3 || damage < 20)
		return; // No pain anims in nightmare. mxd. Ignore low damage
...
// Lazarus: We can do better than that... see below
	...
	// Check to see that we can trace to the player before we start tossing grenades around.
	...
	vhorz[2] = 0;
	const float horz = VectorLength(vhorz);
	const float vertmax = GRENADE_VELOCITY_SQUARED / (2 * sv_gravity->value) - 0.5 * sv_gravity->value * horz * horz / GRENADE_VELOCITY_SQUARED;
	if (dir[2] > vertmax) 
		return false;

	// Lazarus: Make sure there's a more-or-less clear flight path to target
	// Rogue checked target origin, but if target is above gunner then the trace would almost always hit the platform the target was standing on
	VectorCopy(self->enemy->s.origin,target);
	target[2] = self->enemy->absmax[2];
	trace_t tr = gi.trace(start, vec3_origin, vec3_origin, target, self, MASK_SHOT);
	if (tr.ent == self->enemy || tr.fraction == 1)
		return true;

	// Repeat for feet... in case we're looking down at a target standing under, for example, a short doorway
...
void gunner_duck_down(edict_t *self)
{
	...
	if (skill->value >= 2 && random() > 0.5f && gunner_grenade_check(self)) // Lazarus: Added check for goodness of grenade firing
		GunnerGrenade(self);
...
void GunnerFire (edict_t *self)
{
	...
	const int flash_number = MZ2_GUNNER_MACHINEGUN_1 + (self->s.frame - FRAME_attak216);
...
/*	if (skill->value < 2)
	...
	// Lazarus - take into account (sort of) feature of adding shooter's velocity to grenade velocity
...
void gunner_attack(edict_t *self)
{
	...
	{
		// Lazarus: Added bit of logic to check whether grenades are good to fire, and made it more likely to fire grenades on harder skill levels
...
void gunner_refire_chain(edict_t *self)
{
	if (self->enemy->health > 0 && visible(self, self->enemy) && random() <= 0.5)
		self->monsterinfo.currentmove = &gunner_move_fire_chain;
	else
...
void SP_monster_gunner (edict_t *self)
{
	...
	if (self->health < 0)
	{
		mmove_t	*deathmoves[] = { &gunner_move_death, NULL };
