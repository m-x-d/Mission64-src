...
void SelectStartWeapon (gclient_t *client, int style)
{
	...
	// If style is non-zero, first clear player inventory of all weapons and ammo that might have been passed over through
	// target_changelevel or acquired when previously called by InitClientPersistant
	if (style)
	{
...
float PlayersRangeFromSpot(edict_t *spot)
{
	...
	for (int n = 1; n <= maxclients->value; n++)
	{
...
void CopyToBodyQue(edict_t *ent)
{
	...
	edict_t *body = &g_edicts[(int)maxclients->value + level.body_que + 1];
...
void ClientBegin (edict_t *ent)
{
	...
	{
		// a spawn point will completely reinitialize the entity except for the persistant data that was initialized at ClientConnect() time
		G_InitEdict(ent);
		ent->classname = "player";
		InitClientResp(ent->client);
		PutClientInServer(ent);
...
float PM_CmdScale(usercmd_t *cmd)
{
	...
	const float total = sqrt(cmd->forwardmove * cmd->forwardmove + cmd->sidemove * cmd->sidemove + cmd->upmove * cmd->upmove);
	const float scale = max / total;
...
void ClientSpycam(edict_t *ent)
{
	...
	if (is_actor && !camera->enemy)
	{
		if (abs(client->ucmd.forwardmove) > 199 && camera->groundentity)
		{
			...
			if (dist > 8)
			{
				...
				camera->movetarget = thing;
				camera->goalentity = thing;
				camera->monsterinfo.old_leader = NULL;
			...
			{
				camera->monsterinfo.aiflags &= ~AI_CHASE_THING;
				camera->movetarget = NULL;
				camera->goalentity = NULL;
				G_FreeEdict(thing);
				camera->vehicle = NULL;
				actor_stand(camera);
			}
	...
	if (camera->viewer == ent)
	{
		...
		{
			// Give game a bit of time to catch up after player causes ucmd pitch angle to roll over... otherwise
			// we'll hit on the above test even though player hasn't hit +lookup/+lookdown
	...
	if (tr.fraction < 1.0)
	{
		...
		float dist = VectorNormalize(dir) - 2;
		dist = max(0, dist);
...
void ClientThink (edict_t *ent, usercmd_t *ucmd)
{
	...
	if (!client->use && ucmd->buttons & BUTTON_USE)
	{
		...
		else
		{
			...
			if (viewing && viewing->classname)
			{
				...
				else if (!Q_stricmp(viewing->classname, "func_monitor") && (range <= 100 || (client->chasetoggle && range <= client->zoom + 160.00)) )
				{
	...
	if (client->chasetoggle && !client->push)
	{
		...
		edict_t *viewing = LookingAt(ent, 0, intersect, &range);
		if (!(viewing && viewing->classname && (Q_stricmp(viewing->classname, "func_monitor") || Q_stricmp(viewing->classname, "func_pushable")) && range <= 100))
		{
	...
	if (client->zooming)
	{
		...
		{
			client->ps.fov = min(client->original_fov, client->ps.fov + client->frame_zoomrate);
			client->zoomed = (client->ps.fov < client->original_fov);
		}
	}
			...
			if (client->jetpack_framenum + client->pers.inventory[fuel_index] > level.framenum)
			{
				...
				else
				{
					...
					if (ucmd->upmove == 0)
					{
						...
						const float g_max = 0.75 * sv_gravity->value;
						float gravity = g_max * (level.framenum - client->jetpack_last_thrust) / 20;
						gravity = min(g_max, gravity);
...
#ifdef JETPACK_MOD
		if (client->jetpack && jetpack_weenie->value && pm.groundentity && Jet_AvoidGround(ent))
			pm.groundentity = NULL; // We are no longer on ground
#endif
...
// MUD - "correct" Pmove physics
		if (pm.waterlevel && ent->in_mud)
		{
			...
			if (pm.waterlevel == 1)
			{
				frac = MUD1BASE + MUD1AMP * sin((float)(level.framenum % 10) / 10.0 * M_PI2);
			...
			else if (pm.waterlevel == 2)
			{
				...
				point[2] += ent->maxs[2];
				VectorSet(end, point[0], point[1], oldorigin[2] + ent->mins[2]);
				...
				if (dist <= 10)
					...
					frac = MUD3 + (dist - 10) / 22.0 * (MUD1BASE - MUD3);
...
// end MUD
		...
		const qboolean canjump = (ent->groundentity && !pm.groundentity && pm.cmd.upmove >= 10 && pm.waterlevel == 0); //mxd
		
		if (canjump && !client->jetpack)
		{	
...
//ZOID
		...
		// touch other objects. Lazarus: but NOT if game is frozen
		if (!level.freeze)
		{
	...
	if (client->latched_buttons & BUTTONS_ATTACK && !(ctf->value && ent->movetype == MOVETYPE_NOCLIP)) //ZOID +ctf
	{
...
void ClientBeginServerFrame(edict_t *ent)
{
	...
	if (!client->weapon_thunk && !client->resp.spectator && !(ctf->value && ent->movetype == MOVETYPE_NOCLIP) ) //ZOID +ctf
		Think_Weapon(ent);
	else
	...
	if (ent->deadflag)
	{
		...
		if (level.time > client->respawn_time)
		{
			...
			const int buttonMask = (deathmatch->value ? BUTTONS_ATTACK : -1);
	...
	if (!deathmatch->value && !client->spycam && !visible(ent, PlayerTrail_LastSpot()))
