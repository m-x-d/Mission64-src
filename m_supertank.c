...
{
	const int sound_search = (random() < 0.5 ? sound_search1 : sound_search2);
...
/*void supertank_forward(edict_t *self) //mxd. Never used
{
...
void supertank_reattack1(edict_t *self)
{
	if (visible(self, self->enemy) && random() < 0.9)
...
void supertank_pain(edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (skill->value >= 2 && self->s.frame >= FRAME_attak2_1 && self->s.frame <= FRAME_attak2_14)
		return;
...
void supertankMachineGun(edict_t *self)
{
	...
	const int flash_number = MZ2_SUPERTANK_MACHINEGUN_1 + (self->s.frame - FRAME_attak1_1);
...
void SP_monster_supertank(edict_t *self)
{
	...
	if (/*(self->spawnflags & SF_MONSTER_SPECIAL) &&*/ self->style) //mxd. Why the patching was done only when SF_MONSTER_SPECIAL flag was set?
	{
...
//	self->s.sound = gi.soundindex("bosstank/btkengn1.wav");
	...
	if (self->health < 0)
	{
		mmove_t *deathmoves[] = { &supertank_move_death, NULL };
