...
SF 1  = START_ON
   2  = STOP_AT_TOP
   8  = SLOW  (internal use only, used to slow a pendulum to a stop after being blocked)
...
void pendulum_blocked (edict_t *self, edict_t *other)
{
	...
	// Since this routine is called in response to being blocked, the current s.angles is for the LAST frame, not the proposed
	// angles for the current frame. Since we're basically overriding normal physics here, go ahead and move to new location. This
	// means that later on we have to move blocker NOW rather than relying on its velocity to get it out of the way.
	// BUT... trouble is doing this with players ends up giving goofy direction in some cases. For players/monsters, use old
	...
	float speed = fabs(self->avelocity[ROLL]) * M_PI / 180.0 * self->radius;
	...
	if (other->client || (other->svflags & SVF_MONSTER))
	{
		...
		// Normal kickback takes too long to take effect and allows embedment. Move the blocker NOW.
	...
	{
		// Other is most likely a func_pushable, since almost all other bmodels aren't clipped to MOVETYPE_PUSH
		vec3_t		org, mins, maxs;
		vec3_t		vn2;
		qboolean	block = false;
		const float	attenuation = self->attenuation;	// coefficient of restitution
		const float	m = (float)other->mass / (float)self->mass;
		const float	v11 = speed;
		float		v12, v22;
		float		new_rspeed;
		...
		// If pendulum hits crate from above and crate is on the ground, destroy the crate.
		// This may not be realistic, but there's really no other way since if we stop the pendulum we'd then have to 
		...
		if (attenuation > 0)
		{
			float v21 = VectorLength(other->velocity);
			if (v21 > 0)
			{
			...
			v22 = (attenuation * (v11 - v21) + v11 + m * v21) / (1.0 + m);
			v12 = v22 + attenuation * (v21 - v11);
		}
		else //TODO: mxd. Attenuation is set in [>0 .. 1] range in SP_func_pendulum, so how is this supposed to be triggered?
		{
			v12 = v11;
		...
		{
			// Reverse rotation.
			new_rspeed = fabsf(v12) / (M_PI / 180.0 * self->radius);
			if (self->avelocity[ROLL] > 0)
		...
		else
		{
			...
			new_rspeed = v12 / (M_PI / 180.0 * self->radius);
			if (self->avelocity[ROLL] > 0)
				self->avelocity[ROLL] = new_rspeed;
			else
				self->avelocity[ROLL] = -new_rspeed;
		...
		const float sgor = sqrtf((float)sv_gravity->value / self->radius);
		float wave = fabsf(self->avelocity[ROLL] / (angles[ROLL] * sgor));
		wave = atanf(wave);
		...
		const float time = wave / sgor;
		self->startframe = level.framenum - time * 10.0;
		self->moveinfo.start_angles[ROLL] = -fabsf(angles[ROLL] / cosf(wave));

		// Now we know the new pendulum velocity and crate velocity, *assuming* nothing else is in the way.
		// Now check to see if crate hits anything else.

		VectorAdd(other->s.origin, other->origin_offset, org);
		VectorMA(org, FRAMETIME, new_velocity, new_origin);

		// Temporarily make crate nonsolid so we can ignore pendulum in our trace (rather than crate)
		other->solid = SOLID_NOT;
		gi.linkentity(other);
		...
		if (trace.fraction < 1.0)
		{
			...
			{
				const float	delta = FRAMETIME * VectorLength(new_velocity);

				// If a player or monster is in the way of the crate, AND the pendulum speed is > 100, throw 'em out of the way.
			...
			{
				block = true;
				VectorScale(vec, 10.0, other->velocity);
		...
		// moving VERY slowly and we need to reverse direction NOW to prevent embedment.
		if (!block)
		{
			if (fabsf(v12) < 100)
			{
...
void pendulum_rotate (edict_t *self)
{
	...
	if (self->spawnflags & SF_PENDULUM_SLOW)
	{
		...
		{
			const float next_angle = self->s.angles[ROLL] + self->avelocity[ROLL] * FRAMETIME;
	...
	{
		const float old_velocity = self->avelocity[ROLL];

		if (!self->startframe)
			self->startframe = level.framenum;

		const float sgor = sqrtf((float)sv_gravity->value / self->radius);
		const float wave = sgor * (level.framenum - self->startframe) * 0.1;
		const float this_angle = self->moveinfo.start_angles[ROLL] * cosf(wave);
		self->avelocity[ROLL] = -self->moveinfo.start_angles[ROLL] * sgor * sinf(wave);

		if ((self->spawnflags & SF_PENDULUM_STOPPING) && cosf(wave) > 0.0)
		{
...
void pendulum_use(edict_t *self, edict_t *other, edict_t *activator)
{
	...
	else
	{
		...
		if (self->delay > 0)
		{
			float delay = self->delay * M_PI2 * sqrtf(self->radius / (float)sv_gravity->value);
		...
		else
		{
			...
			{
				float t = acosf(self->s.angles[ROLL] / self->moveinfo.start_angles[ROLL]);
				t /= sqrtf((float)sv_gravity->value / self->radius);
...
// Mostly copied from func_explosive_explode. We can't use that function because the origin is a bit different.
...
void SP_func_pendulum (edict_t *ent)
{
	...
	// This is the damage delivered by the pendulum at max speed. Convert to a damage scale used in our damage equation.
	const float max_speed = ent->moveinfo.distance / 2 * M_PI / 180.0 * sqrtf((float)sv_gravity->value * ent->radius);
	if (max_speed <= 200.0)
	{
		ent->dmg = 0;
	}
	...
	{
		const float dmg = (float)ent->dmg * 100.0 / (max_speed - 200.0);
