...
{
	if (!ent->client || !ent->client->textdisplay)
		return;
...
void Text_BuildDisplay(texthnd_t *hnd)
{
	...
	char *p1 = hnd->buffer + hnd->start_char;
	char *p3 = hnd->buffer + hnd->size - 1;
	if (hnd->curline > 0)
	{
...
void Text_Update(edict_t *ent)
{
	...
	texthnd_t *hnd = ent->client->textdisplay;
	if (hnd->last_update + 2 * FRAMETIME > level.time)
		return;

	hnd->last_update = level.time;


	const int x0 = (35 - hnd->page_width) * 4;
	const int y0 = (22 - hnd->page_length) * 4;

	if (!(hnd->flags & 2))
		sprintf(string, "xv %d yv %d picn %s ", x0, y0, hnd->background_image);
...
void Text_Next(edict_t *ent)
{
	...
	int displayed_lines = hnd->page_length;
	if (hnd->nlines > hnd->page_length)
		displayed_lines--;

	if (hnd->curline + displayed_lines + 1 < hnd->nlines)
	{
		const int current = hnd->curline;
...
void Do_Text_Display(edict_t *activator, int flags, char *message)
{
	...
	texthnd_t *hnd = gi.TagMalloc(sizeof(*hnd), TAG_LEVEL);
	...
	if (flags & 1)
	{
		...
		const int textsize = gi.LoadFile(textname, (void **)&readbuffer);
	...
	while (p1 < hnd->buffer + hnd->size)
	{
		...
		qboolean linebreak = false;

		if (line_length > hnd->page_width)
		{
			if (*p1 == 32)
			{
				// We're at a space... good deal, just replace space with a line-break 0 and move on
				*p1 = 0;
				hnd->nlines++;
				linebreak = true;
			}
			else
			{
				...
				if (!linebreak)
				{
					// Must be an ugly Mad Dog test trying my patience - say a 40-character line with no spaces. Back up one space, add a hyphen then a 0.
					hnd->size += 2;
		...
		if (linebreak && alt)
		{
			// We broke a line and the line was green text. Insert another '*' at beginning of next line
			hnd->size += 1;
			if (hnd->size > hnd->allocated)
			{
				hnd->allocated += 128;
		...
		{
			// We broke a line and the line had other than left justification. Insert another '\c' or '\r' at beginning of next line
			hnd->size += 2;
			if (hnd->size > hnd->allocated)
			{
				hnd->allocated += 128;
		...
		if (!linebreak && *p1 == 0 && p1 < hnd->buffer + hnd->size - 1 && line_length < hnd->page_width)
		{
			// Don't do this if 2 consecutive 0's are found (end of paragraph) or if 1st character in next line is '*' or '\'
			p2 = p1;
			p2--;
