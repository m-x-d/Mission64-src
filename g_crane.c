...
void Moving_Speaker_Think(edict_t *speaker)
{
	...
	if (speaker->spawnflags & 15)
	{
		...
		/*if ((speaker->spawnflags & 8) && (!speaker->owner->groundentity || !speaker->owner->activator || !speaker->owner->activator->client)) // (not on the ground || not "activated" by anything || not activated by client)
		{
			moved = false;
		}
		else
...
edict_t *CrateOnTop (edict_t *from, edict_t *ent)
{
	...
	{
		if (from == ent || !from->inuse || from->movetype != MOVETYPE_PUSHABLE)
			continue;
		...
		const float maxdist = VectorLength(ent->velocity) * FRAMETIME + 2.0f;
		if (fabsf(from->absmin[2] - ent->absmax[2]) > maxdist)
			continue;
...
void Cargo_Float_Up (edict_t *cargo)
{
	...
	if (cargo->absmax[2] + cargo->velocity[2] * FRAMETIME >= cargo->crane_hook->absmin[2] - CARGO_BUFFER)
	{
...
{
	const float length = cable->s.origin[2] - cable->crane_hook->absmax[2];
	int frame = (int)(length / CABLE_SEGMENT);

	if ((frame + 1) * CABLE_SEGMENT < length) 
		frame++;

	cable->s.frame = clamp(frame, 0, 19);
}
...
void SetSpotlightLength(edict_t *hook)
{
	...
	const trace_t tr = gi.trace(start, NULL, NULL, end, hook, MASK_SOLID);
...
void Crane_Move_Done (edict_t *ent)
{
	if (!Q_stricmp(ent->classname, "crane_hook"))
	{
		...
		if (ent->crane_cargo)
		{
			ent->crane_cargo->s.origin[2] += ent->absmin[2] - CARGO_BUFFER - ent->crane_cargo->absmax[2];
			gi.linkentity(ent->crane_cargo);
		}

		// ... and finally, stop cable and move to correct position
		edict_t *cable = ent->crane_cable;
		...
		edict_t *light = ent->crane_light;
		if (light)
		{
			VectorClear(light->velocity);
			light->think = crane_light_off;
			light->nextthink = level.time + 1.0f;
			gi.linkentity(light);
		}
	}
...
//	        if (ent->movetype == MOVETYPE_PUSHABLE)
	if (!Q_stricmp(ent->classname, "func_pushable"))
	{
		...
		// Check to see if any OTHER crates are stacked on this one. If so, adjust their position and velocity as well.
		edict_t *e = NULL;
...
void Crane_Stop(edict_t *control)
{
	...
	if (control->crane_hook->crane_cargo) 
		Crane_Move_Done(control->crane_hook->crane_cargo);
}
...
qboolean Crane_Hook_Bonk(edict_t *hook, int axis, int dir, vec3_t bonk)
{
	...
	edict_t *cargo = hook->crane_cargo;
	...
	trace_t tr = gi.trace(start, mins, maxs, end, cargo, MASK_PLAYERSOLID);
	if (tr.fraction < fraction && tr.ent != hook->crane_beam && tr.ent != hook->crane_hoist && tr.ent != cargo )
	{
	...
	if (cargo)
	{
		...
		if (tr.fraction < cargo_fraction && tr.ent != hook->crane_beam && tr.ent != hook->crane_hoist && tr.ent != hook)
		{
...
void Crane_blocked (edict_t *self, edict_t *other)
{
	...
	{
		// treat func_pushable like a world brush - attempt to stop crane
...
void Crane_Move_Final (edict_t *ent)
{
	if (ent->crane_control->activator->client->use)
	{
		...
		float bonk_distance = ent->crane_control->crane_increment * (ent->crane_bonk - ent->absmin[ent->crane_dir]);
...
void G_FindCraneParts()
{
	...
	{
		if (!e->inuse || !e->classname || Q_stricmp(e->classname, "crane_control"))
			continue;

		edict_t *control = e;
		edict_t *beam = G_Find(NULL, FOFS(targetname), control->target);
		if (!beam)
		{
			gi.dprintf("Crane_control with invalid target '%s'\n", control->target);
			G_FreeEdict(control);
		...
		edict_t *p1 = G_Find(NULL, FOFS(targetname), beam->pathtarget);
		if (!p1) //mxd
		{
			gi.dprintf("Invalid pathtarget '%s' for crane_beam '%s'\n", beam->pathtarget, control->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
		...
		if (!p1->target)
		{
			gi.dprintf("Only one path_corner in pathtarget sequence for crane_beam '%s'\n(two are required)\n", control->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
			G_FreeEdict(p1);
		...
		edict_t *p2 = G_Find(NULL, FOFS(targetname), p1->target);
		if (!p2) //mxd
		{
			gi.dprintf("Missing second path_corner '%s' for crane_beam '%s'\n", p1->target, control->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
			G_FreeEdict(p1);
		...
		edict_t *hoist = G_Find(NULL, FOFS(targetname), beam->target);
		if (!hoist)
		{
		...
		if (!p1->target)
		{
			gi.dprintf("Only one path_corner in pathtarget sequence for crane_hoist '%s'\n(two are required)\n", beam->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
			G_FreeEdict(hoist);
			G_FreeEdict(p1);
		...
		if (!p2) //mxd
		{
			gi.dprintf("Missing second path_corner '%s' for crane_hoist '%s'\n", p1->target, beam->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
			G_FreeEdict(hoist);
			G_FreeEdict(p1);
		...
		if (beam->speaker)
			beam->speaker->spawnflags = (direction ? 2 : 1);

		if (hoist->speaker)
			hoist->speaker->spawnflags = (direction ? 1 : 2);

		edict_t *hook = G_Find(NULL, FOFS(targetname), hoist->target);
		if (!hook)
		{
			gi.dprintf("Crane hoist '%s' with invalid target '%s'\n", beam->target, hoist->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
			G_FreeEdict(hoist);
			G_FreeEdict(p1); //mxd
		...
		// We use a trick here... since hook by definition cannot be moving unless control is on, then with control off
		// we set hook speaker spawnflag to require hook to be moving to play
		if (hook->speaker)
			hook->speaker->spawnflags = 4; //mxd. Play sound when moving on Z axis
			//hook->speaker->spawnflags = 1 - (control->spawnflags & 1);
			
		// Get offset from hook origin to hoist origin, so we can correct timing problems
		...
		if (!p1) //mxd
		{
			gi.dprintf("Invalid pathtarget '%s' for crane_hook '%s'\n", hook->pathtarget, hoist->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
			G_FreeEdict(hoist);
			G_FreeEdict(hook);
		...
		if (!p1->target)
		{
			gi.dprintf("Only one path_corner in pathtarget sequence for crane_hook '%s'\n(two are required)\n", hoist->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
			G_FreeEdict(hoist);
			G_FreeEdict(hook);
		...
		if (!p2) //mxd
		{
			gi.dprintf("Missing second path_corner '%s' for crane_hook '%s'\n", p1->target, hoist->target);
			G_FreeEdict(control);
			G_FreeEdict(beam);
			G_FreeEdict(hoist);
			G_FreeEdict(hook);
		...
		if (hook->crane_cable == NULL)
		{
			...
			const float length = hoist->absmax[2] - 1 - hook->absmax[2];
			auto frame = (int)(length / CABLE_SEGMENT);
			if ((frame + 1) * CABLE_SEGMENT < length)
				frame++;
		...
		// If control is NOT onboard, move beam speaker (if any) to end of beam closest to control
...
{
	// Adjust speed so that travel time is an integral multiple of FRAMETIME
	if (ent->moveinfo.remaining_distance > 0)
	{
...
void crane_control_action(edict_t *control, edict_t *activator, const vec3_t point)
{
	int dir;
	int column;
	vec3_t  center, v;
	...
	// First make sure player (activator) is on the panel side of the control panel
	// Also get center point of panel side
	switch (control->style)
	{
	case 0:
		if (activator->s.origin[0] > control->absmax[0])
			return;
		...
		break;
	case 1:
		if (activator->s.origin[1] > control->absmax[1])
			return;
		...
		break;
	case 2:
		if (activator->s.origin[0] < control->absmin[0])
			return;
		...
		break;
	case 3:
		if (activator->s.origin[1] < control->absmin[1])
			return;
		center[0] = (control->absmin[0] + control->absmax[0]) / 2;
	...
	edict_t *beam = control->crane_beam;
	edict_t *cable = control->crane_cable;
	edict_t *hoist = control->crane_hoist;
	edict_t *hook = control->crane_hook;
	edict_t *cargo = hook->crane_cargo;

	if (cargo)
		cargo->gravity = 0.0f; // reset after making it float up, otherwise things get jammed up
	...
	switch (control->style)
	{
		case 1:	 column = (4 * (point[0] - control->absmin[0])) / (control->absmax[0] - control->absmin[0]); break;
		case 2:	 column = (4 * (point[1] - control->absmin[1])) / (control->absmax[1] - control->absmin[1]); break;
		case 3:	 column = (4 * (point[0] - control->absmax[0])) / (control->absmin[0] - control->absmax[0]); break;
		default: column = (4 * (point[1] - control->absmax[1])) / (control->absmin[1] - control->absmax[1]); break;
	}

	column = clamp(column, 0, 3);

	// adjust for controller facing beam movement direction
	if (beam->movedir[0] > 0 && (control->style == 0 || control->style == 2) && (column == 0 || column == 1))
	{
		column = 1 - column;
		row = 1 - row;
	}

	if (beam->movedir[1] > 0 && (control->style == 1 || control->style == 3) && (column == 0 || column == 1))
	{
		column = 1 - column;
		row = 1 - row;
	}
	...
	case 0:
		...
		//==================
		if (row)
			control->crane_increment = ((control->style == 0 || control->style == 1) ?  1 : -1); // hoist away
		else
			control->crane_increment = ((control->style == 0 || control->style == 1) ? -1 :  1); // hoist toward
		...
		hoist->crane_dir = dir;
		hoist->moveinfo.remaining_distance = control->crane_increment *	(hoist->crane_bonk - hoist->absmin[dir]);
		if (hoist->moveinfo.remaining_distance <= 0)
			return;
		...
		hook->crane_dir = dir;
		hook->crane_bonk = hoist->crane_bonk + hook->absmin[dir] - hoist->absmin[dir];
		hook->crane_control = control;
		memcpy(&hook->moveinfo, &hoist->moveinfo, sizeof(moveinfo_t));

		cable->crane_dir = dir;
		cable->crane_bonk = hoist->crane_bonk + cable->absmin[dir] - hoist->absmin[dir];
		cable->crane_control = control;
		...
		if (cargo)
		{
			...
			cargo->crane_dir = dir;
			cargo->crane_bonk = hoist->crane_bonk + cargo->absmin[dir] - hoist->absmin[dir];
	...
	case 1:
		...
		if (row)
			control->crane_increment = ((control->style == 0 || control->style == 3) ? 1 : -1); // left arrow
		else
			control->crane_increment = ((control->style == 0 || control->style == 3) ? -1 : 1); // right arrow
		...
		beam->crane_dir = dir;
		beam->moveinfo.remaining_distance = control->crane_increment * (beam->crane_bonk - beam->absmin[dir]);
		if (beam->moveinfo.remaining_distance <= 0)
			return;
		...
		if (beam->crane_onboard_control)
		{
			...
			beam->crane_onboard_control->crane_bonk = beam->crane_bonk + beam->crane_onboard_control->absmin[dir] -	beam->absmin[dir];
	...
	case 2:
		...
		else
		{
			...
			if (cargo)
			{
				...
				pt[2] = cargo->absmin[2] - 0.125f;

				const int content = gi.pointcontents(pt);
		...
		hook->moveinfo.remaining_distance = control->crane_increment * (hook->crane_bonk - hook->absmin[hook->crane_dir]);
		...
		VectorSet(hook->moveinfo.dir, 0.0f, 0.0f, (float)control->crane_increment);
		hook->crane_control = control;
	...
	case 3:
		...
		if (row)
		{
			...
			VectorScale(mins, 0.3333f, mins);
			VectorScale(maxs, 0.3333f, maxs);
			// end 06/03/00 change

			const trace_t tr = gi.trace(start, mins, maxs, end, hook, MASK_SOLID);
			if (tr.fraction < 1 && tr.ent && tr.ent->classname && tr.ent->movetype == MOVETYPE_PUSHABLE)
			{
				float zdist = hook->absmin[2] - tr.ent->absmax[2];
				if (zdist > MAX_PICKUP_DISTANCE)
				{
				...
				// run a trace from top of cargo up... if first entity hit is NOT the hook, we can't get there from here.
				if (zdist > 0)
				{
					VectorMA(tr.ent->mins, 0.5f, tr.ent->size, start);
					start[2] = tr.ent->maxs[2];
					VectorCopy(tr.ent->size, mins);
					VectorScale(mins, -0.5f, mins);
					...
					const trace_t tr2 = gi.trace(start, mins, maxs, end, hook, MASK_SOLID);
				...
				zdist -= CARGO_BUFFER;   // leave a buffer between hook and cargo
				...
				{
					gi.positioned_sound(start, cargo, CHAN_VOICE, gi.soundindex("tank/thud.wav"), 1, 1, 0);
				}
			}
			else
			{
...
void SpawnSpeaker(edict_t *self)
{
	if (self->noise_index && !VectorLength(self->s.origin)) //TODO: (mxd): why !VectorLength(self->s.origin) condition? 
	{
		...
		speaker->spawnflags = 7;
		self->speaker = speaker;
		VectorAdd(self->absmin, self->absmax, speaker->s.origin);
		VectorScale(speaker->s.origin, 0.5f, speaker->s.origin);
...
void SP_crane_hook (edict_t *self)
{
	...
	self->solid = SOLID_BSP;
	self->movetype = MOVETYPE_PUSH;
	self->noise_index = (st.noise ? gi.soundindex(st.noise) : 0);
...
void crane_reset_go (edict_t *temp)
{
	...
	if (control->crane_hook->crane_cargo)
		Crane_Move_Begin(control->crane_hook->crane_cargo);
...
void crane_reset_use(edict_t *self, edict_t *other, edict_t *activator)
{
	...
	edict_t *crane = G_Find(NULL, FOFS(targetname), self->target);
	if (!crane)
	{
	...
	edict_t *control = crane->crane_control;
	control->activator = activator;
	...
	if (beam->speaker && beam->crane_onboard_control)
	{
		...
		VectorAdd(beam->crane_onboard_control->absmin, beam->crane_onboard_control->absmax, beam->speaker->s.origin);
		VectorScale(beam->speaker->s.origin, 0.5, beam->speaker->s.origin);
		VectorSubtract(beam->speaker->s.origin, beam->crane_onboard_control->s.origin, beam->speaker->offset);
	...
	VectorSet(beam->moveinfo.dir, beam->movedir[0] * control->crane_increment, beam->movedir[1] * control->crane_increment, 0);
	beam->crane_control = control;
