...
restart:
		...
		const qboolean is_monster = (e->svflags & SVF_MONSTER); //mxd
		if (is_monster && e->health <= e->gib_health)
		{
		...
		{
			e->velocity[2] -= offset[0] * self->avelocity[PITCH] * M_PI / 180.0;
			e->velocity[0] += offset[2] * self->avelocity[PITCH] * M_PI / 180.0;
		}

		if (self->avelocity[YAW] != 0)
		{
			e->velocity[0] -= offset[1] * self->avelocity[YAW] * M_PI / 180.0;
			e->velocity[1] += offset[0] * self->avelocity[YAW] * M_PI / 180.0;
		}

		if (self->avelocity[ROLL] != 0)
		{
			e->velocity[1] -= offset[2] * self->avelocity[ROLL] * M_PI / 180.0;
			e->velocity[2] += offset[1] * self->avelocity[ROLL] * M_PI / 180.0;
		}
		...
		if (self->turn_rider)
		{
			...
			{
				const float cy = cos((e->s.angles[1] - delta_angles[1]) * M_PI / 180.0);
				const float sy = sin((e->s.angles[1] - delta_angles[1]) * M_PI / 180.0);
			...
			{
				// Not a monster/actor. We want monsters/actors to be able to turn on their own.
				if (!e->do_not_rotate)
				{
					...
					{
						// Brush models always start out with angles=0,0,0 (after G_SetMoveDir). Use more accuracy here
						VectorCopy(self->avelocity, e->avelocity);
					...
					{
						// For point entities, best we can do is apply a delta to the angles. This may result in foulups if anything gets blocked
		...
		if (!Q_stricmp(e->classname, "func_door") || !Q_stricmp(e->classname, "func_button"))
		{
			...
			{
				// Velocities for door/button movement are handled in normal movement routines
				VectorCopy(self->velocity, e->velocity);
		...
		if (amove[YAW])
		{
			...
			if (!Q_stricmp(e->classname, "func_door") || !Q_stricmp(e->classname,"func_button") || e->solid == SOLID_TRIGGER)
			{
				vec3_t p00, p01, p10, p11;

				// Adjust bounding box for yaw
				const float yaw = e->s.angles[YAW] * M_PI / 180.;
...
void spline_calc(edict_t *train, vec3_t p1, vec3_t p2, vec3_t a1, vec3_t a2, float m, vec3_t p, vec3_t a)
{
	...
	// these greatly simplify/speed up equations (make sure m is already assigned a value)
	const float n     = 1.0 - m;
	const float m2    = m * m;
	const float m3    = m2 * m;
	const float n2    = n * n;
	const float n3    = n2 * n;
	const float mn2_3 = m * n2 * 3;
	const float m2n_3 = m2 * n * 3;
	const float mn_2  = m * n * 2;
	...
	const float s = sqrtf(d[0] * d[0] + d[1] * d[1] + d[2] * d[2]) * 0.4;
	...
	// should be optional:
	// first derivative of bezier formula provides direction vector along the curve (equation simplified in terms of m & n)
...
void train_spline(edict_t *self)
{
	...
	if (!(train->spawnflags & TRAIN_SPLINE)) //mxd. V564 The '&' operator is applied to bool type value.
	{
...
void check_reverse_rotation(edict_t *self, const vec3_t point)
{
	vec3_t	vec;
	vec3_t	vnext;
	vec_t	rotation;
...
void Move_Begin(edict_t *ent)
{
	...
	else
	{
		...
		{
			const float frames = floor(ent->moveinfo.remaining_distance / ent->moveinfo.speed / FRAMETIME);
...
void Move_Calc(edict_t *ent, const vec3_t dest, void(*func)(edict_t*))
{
	...
	{
		if (ent->movewith || level.current_entity == ((ent->flags & FL_TEAMSLAVE) ? ent->teammaster : ent))
		{
			Move_Begin(ent);
		}
		else
...
void AngleMove_Begin(edict_t *ent)
{
	...
	const float len = VectorLength(destdelta);
	
	// divide by speed to get time to reach dest
	const float traveltime = len / ent->moveinfo.speed;
	...
	const float frames = floor(traveltime / FRAMETIME);
...
void plat_CalcAcceleratedMove(moveinfo_t *moveinfo)
{
	...
	const float accel_dist = AccelerationDistance(moveinfo->speed, moveinfo->accel);
	float decel_dist = AccelerationDistance(moveinfo->speed, moveinfo->decel);

	if ((moveinfo->remaining_distance - accel_dist - decel_dist) < 0)
	{
		const float f = (moveinfo->accel + moveinfo->decel) / (moveinfo->accel * moveinfo->decel);
...
void plat_Accelerate (moveinfo_t *moveinfo)
{
	...
	if (moveinfo->current_speed == moveinfo->move_speed)
	{
		...
		{
			const float p1_distance = moveinfo->remaining_distance - moveinfo->decel_distance;
			const float p2_distance = moveinfo->move_speed * (1.0f - (p1_distance / moveinfo->move_speed));
	...
	{
		const float old_speed = moveinfo->current_speed;

		// figure simple acceleration up to move_speed
		moveinfo->current_speed = min(moveinfo->speed, moveinfo->current_speed + moveinfo->accel);

		// are we accelerating throughout this entire move?
		if (moveinfo->remaining_distance - moveinfo->current_speed >= moveinfo->decel_distance)
			return;

		// during this move we will accelrate from current_speed to move_speed and cross over the decel_distance; figure the average speed for the entire move
		const float p1_distance = moveinfo->remaining_distance - moveinfo->decel_distance;
		const float p1_speed = (old_speed + moveinfo->move_speed) / 2.0;
		const float p2_distance = moveinfo->move_speed * (1.0 - (p1_distance / p1_speed));
		const float distance = p1_distance + p2_distance;
...
{ 
	if (!ent->think)
		plat_go_down(ent); // already down if ent->think
}
...
void SP_func_rotating(edict_t *ent)
{
	...
	// Lazarus: Why was this removed? Dunno, but we're gonna use our own sound anyway
	...
	{	
		...
		if (ent->accel == 0)
			...
			ent->accel = min(ent->accel, ent->speed);

		if (ent->decel == 0)
			ent->decel = 1;
		else
			ent->decel = min(ent->decel, ent->speed);
	}
...
{
	edict_t *new_origin = G_Find(NULL, FOFS(targetname), ent->pathtarget);
	if (new_origin)	VectorCopy(new_origin->s.origin, ent->s.origin);
...
void movewith_init(edict_t *self)
{
	...
	edict_t *child = G_Find(NULL, FOFS(movewith), self->targetname);
	edict_t *e = self;
	while (child)
	{
		child->movewith_ent = self;

		// Copy parent's current angles to the child. They SHOULD be 0,0,0 at this point for all currently supported parents, but ya never know.
		VectorCopy(self->s.angles, child->parent_attach_angles);
		...
		VectorSubtract(child->s.origin, self->s.origin, child->movewith_offset);
		e->movewith_next = child;
		e = child;
		child = G_Find(child, FOFS(movewith), self->targetname);
	}
}
...
DOORS
  ...
  spawn a trigger surrounding the entire team unless it is already targeted by another
...
void door_go_down(edict_t *self)
{
	...
	else if (strcmp(self->classname, "func_door_rotating") == 0 || strcmp(self->classname, "func_door_rot_dh") == 0)
	{
		self->moveinfo.state = STATE_DOWN;
...
void door_use(edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (self->spawnflags & DOOR_TOGGLE && (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP))
	{
...
void Think_CalcMoveSpeed(edict_t *self)
{
	...
	float min = fabsf(self->moveinfo.distance);
	...
	const float time = min / self->moveinfo.speed;

	// adjust speeds so they will all complete at the same time
	for (ent = self; ent; ent = ent->teamchain)
	{
		const float newspeed = fabsf(ent->moveinfo.distance) / time;
		const float ratio = newspeed / ent->moveinfo.speed;
...
void Think_SpawnDoorTrigger(edict_t *ent)
{
	...
	const int expand = (ent->movewith ? 16 : 60);
...
{
	//TODO: (mxd): this logic is strange...
...
void train_wait(edict_t *self)
{
	...
	if (self->moveinfo.wait)
	{
		...
		else if (self->spawnflags & TRAIN_TOGGLE)  // && wait < 0
		{
			...
			if (!(self->spawnflags & TRAIN_ROTATE_CONSTANT)) //mxd. V564 The '&' operator is applied to bool type value.
...
{
	if (!self || !self->enemy)
		return; //mxd. self->enemy -> !self->enemy

	if (self->enemy->spawnflags & TRAIN_ROTATE)
	{
		// The the train was changed from TRAIN_ROTATE_CONSTANT to TRAIN_ROTATE by a target_change... get da hell outta here.
...
//mxd
float GetAngularVelocity(float velocity, float angle, float idealangle)
{
	if (angle == idealangle)
		return  0;
	...
	if (angle < idealangle)
	{
		const float dist1 = (idealangle - angle) * 10;
		const float dist2 = (360 - idealangle + angle) * 10;

		if (dist1 < dist2)
			velocity = min(dist1, velocity);
		else
			velocity = min(dist2, velocity) * -1;
	}
	else
	{
		const float dist1 = (angle - idealangle) * 10;
		const float dist2 = (360 - angle + idealangle) * 10;

		if (dist1 < dist2)
			velocity = min(dist1, velocity) * -1;
		else
			velocity = min(dist2, velocity);
	}
...
void train_yaw(edict_t *self)
{
	...
	{
		// The the train was changed from TRAIN_ROTATE to TRAIN_ROTATE_CONSTANT by a target_change... get da hell outta here.
		self->think = train_children_think;
	...
	//mxd
	self->enemy->avelocity[YAW] =   GetAngularVelocity(self->enemy->yaw_speed, self->enemy->s.angles[YAW], self->enemy->ideal_yaw);
	self->enemy->avelocity[PITCH] = GetAngularVelocity(self->enemy->pitch_speed, self->enemy->s.angles[PITCH], self->enemy->ideal_pitch);
	self->enemy->avelocity[ROLL] =  GetAngularVelocity(self->enemy->roll_speed, self->enemy->s.angles[ROLL], self->enemy->ideal_roll);
...
again:
	...
	//Knightmare- calc the real target for the train's mins, since that is 1 unit below the corner of the bmodel in all 3 dimensions
	...
	if (self->spawnflags & TRAIN_SPLINE)
	{
		...
		int frames = (int)(10 * self->moveinfo.distance / self->speed);
		frames = max(1, frames);

		const float speed = (10 * self->moveinfo.distance) / (float)frames;
...
void func_train_find(edict_t *self)
{
	...
	self->from = self->to = ent; // Lazarus: spline stuff
	self->target = ent->target;
...
void train_use(edict_t *self, edict_t *other, edict_t *activator)
{
	...
	else
	{
		...
		if (self->spawnflags & TRAIN_SPLINE)
		{
			...
			self->moveinfo.ratio = max(0.0, self->moveinfo.ratio);
		}
...
void SP_func_train(edict_t *self)
{
	...
	else if (self->spawnflags & TRAIN_ROTATE_CONSTANT) //mxd. Let's not require pathtargets when all we want is constant rotation... 
	{
		self->nextthink = level.time + FRAMETIME;
	...
	// Lazarus: TRAIN_SMOOTH forces trains to go directly to Move_Done from Move_Final rather than slowing down (if necessary) for one frame.
...
{
	if (self->movetarget->nextthink) // elevator busy
		return;
...
These can be used, but not touched.
*/
...
void SP_func_door_secret(edict_t *ent)
{
	...
	VectorClear(ent->s.angles);
	const float side = 1.0 - (ent->spawnflags & SECRET_1ST_LEFT);
	...
	const float length = fabsf(DotProduct(forward, ent->size));
...
// box_movestep
//
// Similar to SV_movestep in monster code, but handles falling objects, and doesn't balk at water/lava/slime
//=============
//

qboolean box_movestep(edict_t *ent, const vec3_t move, qboolean relink)
{
...
// push down from a step height above the wished position
	...
	trace_t trace = gi.trace(neworg, mins, maxs, end, ent, MASK_MONSTERSOLID);
...
// 2) calls moving box-specific box_movestep, which allows boxes to fall
//===============
//
...
{
	if (!(ent->flags & FL_SWIM) || ent->waterlevel == 0 || ent->crane_control)
		return;
	...
	for (int i = 0; i < 2; i++)
	{
		if (ent->velocity[i] != 0)
		{
			const float speed = fabs(ent->velocity[i]);
			const float control = max(100, speed);
			float newspeed = speed - (FRAMETIME * control * ent->waterlevel);
			newspeed = max(newspeed, 0) / speed;
...
void box_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	// if other is another func_pushable, AND self is in water, move in proportion to relative masses
	if (other->movetype == MOVETYPE_PUSHABLE)
	{
		...
		if (self->health > 0)
		{
			...
			float delta = VectorLength(impact_v);
			delta = delta * delta * 0.0001f;

			if (delta > 30)
			{
				const int damage = (float)other->mass / (float)self->mass * ((delta - 30) / 2);
				if (damage > 0)
				{
		...
		// 06/03/00 change: If either func_pushable is currently being moved by crane, bail out.
		if (self->crane_control || other->crane_control)
			return;

		// Since func_pushables have a bounding box, impact will ALWAYS be on one of the planes of the bounding box. The "plane" argument isn't always used, but since 
		// all entities involved use a parallelepiped bounding box we can rely on offsets to centers to figure out which side the impact is on.
		...
		int axis = 0;
		if (fabs(v[2]) > fabs(v[axis]))
			axis = 2;
		else if (fabs(v[1]) > fabs(v[axis]))
			axis = 1;

		const float e = 0.5f; // coefficient of restitution
		const float m = (float)other->mass / (float)self->mass;
		const float v11 = self->velocity[axis];
		const float v21 = other->velocity[axis];
		const float v22 = (e * (v11 - v21) + v11 + m * v21) / (1.0 + m);
		const float v12 = v22 + e * (v21 - v11);
		self->velocity[axis] = v12;
		other->velocity[axis] = v22;

		// Assuming frictionless surfaces, momentum of crate is conserved in other two directions (so velocity doesn't change)... BUT we want 
		// to get the bottom crate out from underneath the other one, so we're gonna be a little "creative"
		if (axis == 2)
		{
			if (v[2] > 0)
			{
	...
	// if other is a monster or a player and box is on other's head and moving down, do impact damage
	...
	if (other->client || (other->svflags & SVF_MONSTER))
	{
		...
		if (axis == 2 && v[axis] > 0)
		{
			const float v11 = VectorLength(self->velocity);
			...
			else if (v11 > 0 && v[2] < -0.7f)
			{
				vec3_t	dir, impact_v;
				VectorSubtract(other->velocity, self->velocity, impact_v);
				float delta = VectorLength(impact_v);
				delta = delta * delta * 0.001;
				int damage = (float)self->mass / (float)other->mass * delta;
				
				// always do some minimum amount of damage, or we get boxes on heads, which looks awfully damn odd
				damage = max(2, damage);
				VectorSubtract(self->s.origin, other->s.origin, dir);
				VectorNormalize(dir);
	...
	if (self->groundentity && self->mass > 1000)
		return;

	// if player not pressing use key OR if player in contact with this func_pushable is already pushing something else, do nothing
	if (!other->client->use || (other->client->push != NULL && other->client->push != self))
	{
	...
	if (self->activator && self->activator->client && self->activator != other)
		return;
	...
	// if player isn't on solid ground AND object isn't in water, OR if player is standing on this box, do nothing
	...
	// Now get the offset from the player to the object, and preserve that offset in ClientThink by shifting object as needed.
	...
	other->client->maxvelocity = 20.0 * ratio;

	// Knightmare- don't autoswitch if client-side chasecam is on
#ifdef KMQUAKE2_ENGINE_MOD
	if (tpp_auto->value && (!cl_thirdperson->value || deathmatch->value || coop->value) && !other->client->chasetoggle)
#else
...
void SP_func_pushable(edict_t *self)
{
	...
	switch (self->sounds)
	{
		case 1: self->noise_index = gi.soundindex("tank/thud.wav");	break;
		case 2: self->noise_index = gi.soundindex("weapons/rg_hum.wav"); break;
	...
	// Move up 1 unit so that func_pushable sitting directly on a func_train won't block it's movement (since train has a 1 unit border)
	self->s.origin[2] += 1;
	gi.linkentity(self);
}
...
// amplitude specified by "bob" value and duration of one cycle specified by "duration"
//
void bob_think(edict_t *self)
{
	const int	time = self->duration * 10;

	const float t0 = self->bobframe % time;
	const float t1 = (self->bobframe + 1) % time;

	const float z0 = sin(2 * M_PI * t0 / time);
	const float z1 = sin(2 * M_PI * t1 / time);
	const float delta = self->bob / 2 * (z1 - z0);
...
void pivot_touch(edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	// other is weightless || other is in air || other is not standing on ent
	if (!other->mass || !other->groundentity || other->groundentity != ent)
		return;

	VectorSubtract(ent->s.origin, other->s.origin, offset);
	offset[2] = 0.0; // z offset is irrelevant
...
void pivot_init(edict_t *ent)
{
	...
	if (ent->spawnflags & 1)
	{
		...
		trace_t tr = gi.trace(start, NULL, NULL, end, ent, MASK_SOLID);
		float zmin = (tr.fraction < 1.0 ? tr.endpos[2] : end[2]);

		ent->pos2[PITCH] = asinf((ent->absmin[2] - zmin) / (ent->s.origin[0] - ent->absmin[0]));

		end[0] = start[0] = ent->absmax[0];
		end[2] = ent->absmin[2] - (ent->absmax[0] - ent->s.origin[0]);

		tr = gi.trace(start, NULL, NULL, end, ent, MASK_SOLID);
		zmin = (tr.fraction < 1.0 ? tr.endpos[2] : end[2]);

		ent->pos1[PITCH] = asinf((ent->absmin[2] - zmin) / (ent->absmax[0] - ent->s.origin[0]));
	...
	else
	{
		...
		ent->pos1[ROLL] = asinf((ent->absmin[2] - zmin) / (ent->s.origin[1] - ent->absmin[1]));
		...
		ent->pos2[ROLL] = asinf((ent->absmin[2] - zmin) / (ent->absmax[1] - ent->s.origin[1]));
...
{
	if (!other->takedamage || self->timestamp > level.time)
		return;
...
void SP_func_force_wall(edict_t *ent)
{
	...
	for (int i = 0; i < 3; i++)
		ent->offset[i] = (ent->absmax[i] + ent->absmin[i]) / 2;
...
// from whoever opens it. If opened by a trigger the normal func_door_rotating rotation is used.
//
void swinging_door_killed(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	edict_t *master = self->teammaster;
	if (master->spawnflags & DOOR_TOGGLE && (master->moveinfo.state == STATE_UP || master->moveinfo.state == STATE_TOP))
	{
		// trigger all paired doors
...
{
	edict_t *follow = G_Find(NULL, FOFS(targetname), self->followtarget);
	if (!follow)
	{
	...
	if (self->pathtarget)
	{
		edict_t *new_origin = G_Find(NULL, FOFS(targetname), self->pathtarget);
		if (new_origin)
		{
...
{
	const int pivot = self->spawnflags & 1;	// 1 means "start open" for normal doors, so turn it off temporarily until normal door initialization is done
	self->spawnflags &= ~1;
