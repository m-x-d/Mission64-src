...
void vehicle_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if(vspeed < 200) {
		if(other->mass > self->mass) vspeed *= (float)self->mass/other->mass; //mxd. Cast to float
	...
	points = 100. * (self->mass/2000.0 * vspeed*vspeed/160000); //mxd. 2000 -> 2000.0
	...
	if(gi.pointcontents(new_origin) & CONTENTS_SOLID)
		knockback = (160./500.) * 200. * (self->mass/2000.0 * vspeed*vspeed/160000); //mxd. 2000 -> 2000.0
	else {
		knockback = 0;
		VectorCopy(new_velocity,other->velocity);
		VectorCopy(new_origin,  other->s.origin);
