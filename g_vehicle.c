...
// Not needed, because collisions are effectively prevented by the vehicle physics in g_phys.c.
void vehicle_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	// if vehicle isn't being driven, it can't hurt anybody || can't hurt the driver
	if (!self->owner || other == self->owner || other->takedamage == DAMAGE_NO)
		return;
	...
	vec_t vspeed = VectorLength(self->velocity);
	if (!vspeed)
		return;
	...
	// Lessee... with a mass=2000 vehicle traveling 400 units/sec, give 100 points damage and a velocity of 160 to a 200 mass monster.
	vspeed *= DotProduct(dir, v);
	const vec_t mspeed = VectorLength(other->velocity) * DotProduct(dir, v);
	vspeed -= mspeed;
	...
	if (vspeed < 200)
	{
		if (other->mass > self->mass) 
			vspeed *= (float)self->mass / other->mass; //mxd. Cast to float
		...
		const trace_t tr = gi.trace(new_origin, other->mins, other->maxs, end, self, CONTENTS_SOLID);
		if (tr.startsolid)
		{
			// splat
			T_Damage(other, self, self->owner, dir, self->s.origin, vec3_origin, other->health - other->gib_health + 1, 0, 0, MOD_VEHICLE); 
		}
		else
		{
	...
	other->damage_debounce_time = level.time + 0.2f;
	const vec_t points = 100.0f * (self->mass / 2000.0f * vspeed * vspeed / 160000.0f); //mxd. 2000 -> 2000.0
	// knockback takes too long to take effect. If we can move him w/o throwing him into a solid, do so NOW
	...
	{
		knockback = (160.0f / 500.0f) * 200.0f * (self->mass / 2000.0f * vspeed * vspeed / 160000.0f); //mxd. 2000 -> 2000.0
	}
	else
	{
		knockback = 0;
	...
	T_Damage(other, self, self->owner, dir, self->s.origin, vec3_origin, (int)points, (int)knockback, 0, MOD_VEHICLE);
	gi.linkentity(other);
}
...
void vehicle_think(edict_t *self)
{
	...
	if (DotProduct(forward, self->oldvelocity) < 0) 
		speed = -speed;
	...
	if (self->owner)
	{
		...
		if (self->owner->client->use && level.framenum - self->owner->client->vehicle_framenum > 2)
		{
		...
		{
			float aspeed = 180.0 * speed / (M_PI * self->radius);
			if (self->owner->client->ucmd.sidemove > 0)
				aspeed = -aspeed;
		...
		if (speed != 0)
		{
			const float yaw = self->avelocity[1] * FRAMETIME;
	...
	else
	{
		...
		// No driver
		//
		// if vehicle has stopped, drop it to ground, otherwise slow it down
		if (speed == 0)
		{
		...
		{
			if (!ent->inuse || ent->movetype == MOVETYPE_NOCLIP || !ent->client->use || level.framenum - ent->client->vehicle_framenum <= 2)
				continue;
			...
			if (VectorLength(dir) < 16)
			{
				...
				// Player has taken control of vehicle. Move vehicle up slightly to avoid roundoff collisions
				self->s.origin[2] += 1;
				gi.linkentity(self);
