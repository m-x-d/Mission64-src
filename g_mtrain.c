...
void SP_model_train (edict_t *self)
{
	...
	// Really gross stuff here... translate model_spawn spawnflags to func_train spawnflags. PLAYER_MODEL and NO_MODEL have
	// already been checked in SP_model_spawn and are never re-used, so it's OK to overwrite those.
	...
	// TRAIN_SMOOTH forces trains to go directly to Move_Done from Move_Final rather than slowing down (if necessary) for one frame.
	self->smooth_movement = (self->spawnflags & TRAIN_SMOOTH);
