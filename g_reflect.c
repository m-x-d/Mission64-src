...
//mxd
qboolean WithinBBox(vec3_t p, edict_t *mirror)
{
	if (p[0] < mirror->absmin[0] || p[0] > mirror->absmax[0] ||
		p[1] < mirror->absmin[1] || p[1] > mirror->absmax[1] ||
		p[2] < mirror->absmin[2] || p[2] > mirror->absmax[2])
		return false;
...
void ReflectExplosion(int type, const vec3_t origin)
{
	...
	for (int m = 0; m < level.num_reflectors; m++)
	{
		edict_t *mirror = g_mirror[m];

		if (!mirror->inuse || mirror->spawnflags & SF_REFLECT_OFF)
			continue;

		// Don't reflect explosions (other than BFG) from floor or ceiling, 'cuz there's no way to do it right
...
void ReflectTrail(int type, const vec3_t start, const vec3_t end)
{
	...
	for (int m = 0; m < level.num_reflectors; m++)
	{
		...
		// If p1 is within func_reflect, we assume p2 is also. If map is constructed properly this should always be true.
		if (!WithinBBox(p1, mirror)) //mxd
			continue;
...
void ReflectSparks(int type, const vec3_t origin, const vec3_t movedir)
{
	...
	for (int m = 0; m < level.num_reflectors; m++)
	{
		...
		// Don't reflect in floor or ceiling because sparks are affected by gravity
		if (mirror->style <= 1)
			continue;
...
void DeleteReflection(edict_t *ent, int index)
{
	...
	{
		edict_t	*r = ent->reflection[index];
		if (r)
		{
			if (r->client)
				gi.TagFree(r->client);
...
void AddReflection(edict_t *ent)
{
	...
	for (int i = 0; i < 6; i++)
	{
		qboolean is_reflected = false;
		for (int m = 0; m < level.num_reflectors && !is_reflected; m++)
		{
			edict_t *mirror = g_mirror[m];

			if (!mirror->inuse || mirror->spawnflags & SF_REFLECT_OFF || mirror->style != i)
				continue;
		...
		if (is_reflected)
		{
			...
			if (!(ent->s.modelindex == (MAX_MODELS - 1) && hand->value == 1))
				ent->reflection[i]->s.renderfx |= RF_MIRRORMODEL; // Knightmare added- flip reflected models //TODO: mxd. Remove flag instead if left-handed? 
		#endif
