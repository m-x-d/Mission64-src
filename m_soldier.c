...
{
	if (!(self->spawnflags & SF_MONSTER_AMBUSH) && random() > 0.8)
...
void soldier_pain(edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (skill->value > 0 || damage < 9)
		return; //mxd. No pain anims in medium+; ignore low damage
...
void soldier_fire(edict_t *self, int flash_number)
{
	...
	else
	{
		...
		if (skill->value < 3)
		{
			...
			const float r = crandom() * (1000 - 333 * skill->value);
			const float u = crandom() * (500 - 167 * skill->value);
...
{
	const int sound_sight = (random() < 0.5 ? sound_sight1 : sound_sight2);
	gi.sound(self, CHAN_VOICE, sound_sight, 1, ATTN_NORM, 0);

	if (skill->value > 0 && range(self, self->enemy) >= RANGE_MID && random() > 0.5)
...
void soldier_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	const int n = (skill->integer < 1 ? (rand() % 4) + 1 : rand() % 5);
	//n = rand() % 5;
	if (n == 0)
...
void SP_monster_soldier_x(edict_t *self)
{
	...
	if (self->health < 0)
	{
		mmove_t	*deathmoves[] = { &soldier_move_death1, &soldier_move_death2, &soldier_move_death3, &soldier_move_death4, &soldier_move_death5, &soldier_move_death6, NULL };
