...
static int CheckPowerArmor(edict_t *ent, vec3_t point, vec3_t normal, int damage, int dflags)
{
	...
	if (client)
	{
		...
		if (power_armor_type != POWER_ARMOR_NONE)
			power = client->pers.inventory[cells_index];
	}
	...
	if (power_armor_type == POWER_ARMOR_NONE || !power)
		return 0;
	...
	const int power_used = save / damagePerCell;

	if (client)
		client->pers.inventory[cells_index] -= power_used;
	else
...
static int CheckArmor(edict_t *ent, vec3_t point, vec3_t normal, int damage, int te_sparks, int dflags)
{
	...
	save = min(client->pers.inventory[index], save);
	...
	if ((attacker->client && !(attacker->flags & FL_NOTARGET)) || (attacker->svflags & SVF_MONSTER))
	{
		...
		if ( (targ->svflags & SVF_MONSTER) && (targ->dmgteam) && (targ->health > 0) )
		{
			...
			if (!attacker->dmgteam || strcmp(targ->dmgteam,attacker->dmgteam))
			{
				...
				{
					// Target is a GOOD_GUY, attacked by the player. Allow self-defense, but don't get other dmgteam teammates involved.
					// Special case for misc_actors - if ACTOR_BAD_GUY isn't set, actor stays on the same team and only taunts player
						...
						if (teammate != targ)
						{
							if (teammate->svflags & SVF_MONSTER)
							{
									...
									&& targ->health - damage < 1 && random() < skill->value * 0.25f)
								{
									...
									{
										const trace_t tr = gi.trace(teammate->s.origin, vec3_origin, vec3_origin, targ->s.origin, teammate, MASK_OPAQUE);
										if (tr.fraction == 1.0f)
										{
											// Spawn lightning SFX...
											gi.WriteByte(svc_temp_entity);
											...
											// Play sound
											gi.sound(targ, CHAN_BODY, gi.soundindex("world/x_light.wav"), 1, ATTN_NORM, 0);
...
void M_ReactToDamage(edict_t *targ, edict_t *attacker)
{
	...
	// If targ is currently chasing a "thing" or we're running a hint_path test, he doesn't really care what else is happening
	...
	if (!attacker->client && !(attacker->svflags & SVF_MONSTER) && !is_turret && !(targ->monsterinfo.aiflags & AI_DUCKED))
	{
		if (!targ->movetarget || (targ->movetarget && Q_stricmp(targ->movetarget->classname, "thing")))
		{
			...
			mins[2] = min(0, mins[2] + 16); // max step height? not sure about this
				...
				|| !Q_stricmp(attacker->classname,"func_pushable"))
			{
				...
				for (int i = 0; i < 5; i++)
				{
					...
					const vec_t dist = trace1.fraction * 8192;
			...
			{
				// Send the monster in a direction perpendicular to laser path, whichever direction is closest to current angles
				thing = SpawnThing();
			...
			else
			{
				...
				for (int i = 0; i < 32 && best_dist == 0; i++)
				{
					...
					VectorMA(targ->s.origin, 8192, dir, end);
					trace1 = gi.trace(targ->s.origin, mins, maxs, end, targ, MASK_MONSTERSOLID);
					const trace_t trace2 = gi.trace(trace1.endpos, NULL, NULL, atk, targ, MASK_SOLID);
			...
			vec_t run;
			if (!Q_stricmp(attacker->classname,"func_door") || !Q_stricmp(attacker->classname,"func_pushable"))
				run = 256;
			else
				run = 8192;
			...
			trace1 = gi.trace(targ->s.origin, mins, maxs, end, targ, MASK_MONSTERSOLID);
			const vec_t dist = trace1.fraction * run;
			...
			if (targ->enemy)
				...
				thing->touch_debounce_time = level.time + max(5.0, dist / 50.0f);
	...
	if ((targ->monsterinfo.aiflags & AI_ACTOR) && targ->health < targ->max_health / 3 && attacker->health > attacker->max_health / 2 && attacker->health > targ->health && ai_chicken(targ, attacker))
		return;

	if (attacker == targ || attacker == targ->enemy)
		return;

	// if we are a good guy monster and our attacker is a player or another good guy, do not get mad at them
	...
	if ((targ->monsterinfo.aiflags & AI_TARGET_ANGER) && targ->enemy)
	{
		...
		if (targ->enemy->inuse)
		{
			const float health_ratio = (float)targ->health / (float)targ->max_health;
			if (health_ratio > 0.333 && targ->enemy->inuse)
				return;
		}
	...
	if ((targ->monsterinfo.aiflags & AI_MEDIC) && targ->enemy)
	{
		...
		if (health_ratio > 0.25f && targ->enemy->inuse) 
			return;
	...
	// it's the same base (walk/swim/fly) type and a different classname and it's not a tank (they spray too much), get mad at them
	if (((targ->flags & (FL_FLY | FL_SWIM)) == (attacker->flags & (FL_FLY | FL_SWIM))) 
		&& strcmp(targ->classname, attacker->classname)
		&& strcmp(attacker->classname, "monster_tank")
		&& strcmp(attacker->classname, "monster_supertank")
		&& strcmp(attacker->classname, "monster_makron")
		&& strcmp(attacker->classname, "monster_jorg"))
	{
		// Lazarus: Check IGNORE_SHOTS spawnflag for attacker. If set AND targ and attacker have same GOODGUY setting, don't respond to attacks.
...
{
	//ZOID
	if (ctf->value && targ->client && attacker->client && targ->client->resp.ctf_team == attacker->client->resp.ctf_team && targ != attacker)
		return true;
...
void T_Damage(edict_t *in_targ, edict_t *inflictor, edict_t *in_attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
	...
	edict_t *targ = in_targ;

	if (!targ || !targ->inuse || !targ->takedamage)
		return;

	// Lazarus: If monster/actor is currently being forced to use specific animations due to target_animation, release that control over it.
	...
	// If targ is a fake player for the real player viewing camera, get that player out of the camera and do the damage to him
	...
	// If targ is a remote turret_driver and attacker is a player, replace turret_driver with normal infantry dude and turn TRACK off on corresponding turret_breach
	if (targ->classname && !Q_stricmp(targ->classname, "turret_driver") && (targ->spawnflags & 1) && attacker->client)
	{
	...
	// if enabled you can't hurt teammates (but you can hurt yourself) knockback still occurs
...
// Lazarus: If monster is currently chasing a "thing" and mod is a laser, ignore knockback to give poor guy a chance to vamoose.
...
// figure momentum add
	if (!(dflags & DAMAGE_NO_KNOCKBACK))
	{
		...
		{
			vec3_t	kvel;
			const float mass = max(targ->mass, 50);
...
// do the damage
	if (take)
	{
		...
		if (targ->client)
		{
			if (in_targ != targ)
			{
				// Then player has taken the place of whatever was originally damaged, as in switching from func_monitor usage.
				// Limit damage so that player isn't killed, and make him temporarily invincible
	...
	// add to the damage inflicted on a player this frame the total will be turned into screen blends and view angle kicks at the end of the frame
	if (client)
	{
