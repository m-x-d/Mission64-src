...
qboolean Jet_AvoidGround (edict_t *ent)
{
	...
	const trace_t trace = gi.trace(ent->s.origin, ent->mins, ent->maxs, new_origin, ent, MASK_PLAYERSOLID);

	const qboolean success = (trace.plane.normal[2] == 0);
	if (success)
		ent->s.origin[2] += 0.5; // No ceiling? Then make sure off ground
...
void Jet_ApplyLifting (edict_t *ent)
{
	...
	float delta = sinf((float)((level.framenum % time) * (360.0f / time)) / 180 * M_PI) * amplitude;
	...
	// Before we change origin, check that we dont go into solid
	const trace_t trace = gi.trace(ent->s.origin, ent->mins, ent->maxs, new_origin, ent, MASK_MONSTERSOLID);
...
direction Boosters: the big Main Booster and the smaller up-down and left-right Boosters.
...
void Jet_ApplyJet (edict_t *ent, usercmd_t *ucmd)
{
	...
	// Calculate the direction vectors dependent on viewing direction (length of the vectors forward/right is always 1, the coordinates of
	// the vectors are values of how much youre looking in a specific direction	[if youre looking up to the top, the x/y values are nearly 0 the z value is nearly 1])
	vec3_t forward, right;
	AngleVectors(ent->client->v_angle, forward, right, NULL);
	
	// Run jet only 10 times a second so movement doesn't depend on fps because ClientThink is called as often as possible (fps<10 still is a problem ?)
	if (ent->client->jetpack_nextthink <= level.framenum)
	{
		...
		// If we are moving forward or backward add MainBooster acceleration (60)
		if (ucmd->forwardmove)
		{
			// Are we accelerating backward or forward?
			const float direction = (ucmd->forwardmove < 0 ? -1.0 : 1.0);
		...
		if (ucmd->sidemove)
		{
			...
			const float direction = (ucmd->sidemove < 0 ? -1.0 : 1.0);

			// Add only to x and y acceleration
			const float scale = direction * (jetpack_weenie->value ? 40 : 80);
		...
		if (ucmd->upmove)
		{
			const float scale = (jetpack_weenie->value ? 30 : 45);
			acc[2] += (ucmd->upmove > 0 ? scale : -scale);
		}

		// Now apply some friction dependent on velocity (higher velocity results in higher friction), without acceleration this will reduce the velocity to 0 in a few steps
		ent->velocity[0] *= 0.83;
		ent->velocity[1] *= 0.83;
		ent->velocity[2] *= 0.86;

		// Then accelerate with the calculated values. If the new acceleration for a direction is smaller than an earlier, the friction will reduce the speed
		// in that direction to the new value in a few steps, so if youre flying curves or around corners youre floating a little bit in the old direction
		VectorAdd(ent->velocity, acc, ent->velocity);
		
		// Round velocities (is this necessary?)
		...
		// Bound X and Y velocity so that friction and acceleration dont need to be synced on maxvelocities
		const int maxvelocity = (jetpack_weenie->value ? 300 : 1000);
		ent->velocity[0] = clamp(ent->velocity[0], -maxvelocity, maxvelocity);
