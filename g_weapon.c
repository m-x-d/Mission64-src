...
This is a support routine used when a client is firing a non-instant attack weapon.
...
void check_dodge (edict_t *self, vec3_t start, vec3_t dir, int speed)
{
	...
	const trace_t tr = gi.trace(start, NULL, NULL, end, self, MASK_SHOT);

	if (tr.ent && (tr.ent->svflags & SVF_MONSTER) && tr.ent->health > 0 && tr.ent->monsterinfo.dodge && infront(tr.ent, self))
	{
		vec3_t v;
		VectorSubtract(tr.endpos, start, v);
		const float eta = (VectorLength(v) - tr.ent->maxs[0]) / speed;
...
Fires a single round.  Used for machinegun and chaingun.  Would be fine for pistols, rifles, etc....
=================
*/
...
void blaster_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage)
	{
		const int mod = (self->spawnflags & 1 ? MOD_HYPERBLASTER : MOD_BLASTER);
...
void fire_blaster(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper, int color)
{
	...
	const trace_t tr = gi.trace(self->s.origin, NULL, NULL, bolt->s.origin, bolt, MASK_SHOT);
	if (tr.fraction < 1.0f)
	{
...
void Grenade_Remove_From_Chain (edict_t *grenade)
{
	...
	{
		// "prev_grenade" should always be valid for other than player-thrown grenades that explode in player's hand
...
void Grenade_Explode(edict_t *ent)
{
	...
	if (ent->enemy)
	{
		...
		const float points = ent->dmg - 0.5 * VectorLength(v);
		VectorSubtract(ent->enemy->s.origin, ent->s.origin, dir);

		mod = (ent->spawnflags & 1 ? MOD_HANDGRENADE : MOD_GRENADE);
...
void fire_grenade(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean contact)
{
	...
	// Lazarus - keep same vertical boost for players, but monsters do a better job of calculating aim direction, so throw that out
	if (self->client)
	...
	if (self->enemy && self->enemy->inuse)
	{
		...
		if (tr.fraction == 1)
		{
			...
			if (level.time >= self->starttime && self->starttime > 0)
			{
				...
				{
					// this prevents multiple lockon sounds resulting from monsters firing multiple rockets in quick succession
					if (self->enemy->client)
...
void Rocket_Evade (edict_t *rocket, vec3_t dir, float speed)
{
	...
	trace_t tr = gi.trace(rocket->s.origin, rocket->mins, rocket->maxs, rocket_vec, rocket, MASK_SHOT);
	...
	{
		if (!ent->inuse || !(ent->svflags & SVF_MONSTER) || !ent->takedamage || ent->health <= 0 || rocket->owner == ent)
			continue;
		...
		const float rocket_dist = VectorNormalize(rocket_vec);
		...
		// Find best escape route.
		float best_r = 9999;
		float best_yaw = ent->ideal_yaw; //mxd
		...
		for (int i = 0; i < 8; i++)
		{
			...
			// Estimate of required distance to run. This is conservative.
			const float r = rocket->dmg_radius + rocket_dist * DotProduct(forward, rocket_vec) + ent->size[0] + 16;
			if (r < best_r)
			{
...
void rocket_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	gi.WriteByte(svc_temp_entity);
	const int type = (ent->waterlevel ? TE_ROCKET_EXPLOSION_WATER : TE_ROCKET_EXPLOSION);
	gi.WriteByte(type);
	gi.WritePosition(origin);
	gi.multicast(ent->s.origin, MULTICAST_PHS);
...
void fire_rocket(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage, edict_t *home_target)
{
	...
	if (home_target)
	{
		...
		rocket->endtime = level.time + 8000.0 / speed; //mxd. 8000 -> 8000.0
	...
	else
	{
		...
		rocket->nextthink = level.time + 8000.0 / speed; //mxd. 8000 -> 8000.0
...
void rocket_delayed_start (edict_t *rocket)
{
	if (g_edicts[1].linkcount)
	{
		...
		rocket->nextthink = level.time + 8000.0 / rocket->moveinfo.speed;
		rocket->think = G_FreeEdict;
		gi.linkentity(rocket);
	}
	else
...
void bfg_explode (edict_t *self)
{
	...
	if (self->s.frame == 0)
	{
		...
		{
			if (!ent->takedamage || ent == self->owner || !CanDamage(ent, self) || !CanDamage(ent, self->owner))
				continue;
			...
			float points = self->radius_dmg * (1.0f - sqrtf(dist / self->dmg_radius));
			if (ent == self->owner)
				points = points * 0.5;
...
void bfg_think(edict_t *self)
{
	...
	while ((ent = findradius(ent, self->s.origin, 256)) != NULL)
	{
		...
		//ZOID. Don't target friendly players in CTF
		if (ctf->value && ent->client && self->owner->client &&	ent->client->resp.ctf_team == self->owner->client->resp.ctf_team)
			continue;
...
void fire_bfg(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius)
{
	...
	bfg->owner = self;
	bfg->touch = bfg_touch;
	bfg->nextthink = level.time + 8000.0 / speed; //mxd. 8000 -> 8000.0
...
qboolean AimGrenade (edict_t *self, vec3_t start, const vec3_t target, vec_t speed, vec3_t aim)
{
	...
	if (self->svflags & SVF_MONSTER)
	{
		...
		VectorNormalize(aim);
		yo = from_muzzle[2];
		xo = sqrtf(from_muzzle[0] * from_muzzle[0] + from_muzzle[1] * from_muzzle[1]);
	}
	else
	{
		...
		VectorNormalize(aim);
		yo = from_origin[2];
		xo = sqrtf(from_origin[0] * from_origin[0] + from_origin[1] * from_origin[1]);
	}

	// If resulting aim vector is looking straight up or straight down, we're done. Actually now that I write this down and think about it... should
	// probably check straight up to make sure grenade will actually reach the target.
	if (aim[2] == 1.0 || aim[2] == -1.0)
		return true;

	// Horizontal distance to target from muzzle
	vec_t x = sqrtf(from_muzzle[0] * from_muzzle[0] + from_muzzle[1] * from_muzzle[1]);
	...
	// If flight time is less than one frame, no way grenade will drop much, shoot the sucker now.
	if (t < FRAMETIME)
		return true;

	// In that time, grenade will drop this much:
	float drop = 0.5f * sv_gravity->value * t * t;
	float y = speed * aim[2] * t - drop;
	float v_error = target[2] - start[2] - y;

	// If we're fairly close and we'll hit target at current angle, no need for all this, just shoot it
	if (x < 128 && fabs(v_error) < 16)
		return true;

	float last_error = 100000.0f;
	VectorCopy(aim, last_aim);

	// Unfortunately there is no closed-form solution for this problem, so we creep up on an answer and balk 
	// if it takes more than 10 iterations to converge to the tolerance we'll accept.
	int i;
	for (i = 0; i < 10 && fabsf(v_error) > 4 && fabsf(v_error) < fabsf(last_error); i++)
	{
		last_error = v_error;
		...
		if (!(self->svflags & SVF_MONSTER))
		{
			...
			VectorSubtract(aim_point, start, from_muzzle);
			x = sqrtf(from_muzzle[0] * from_muzzle[0] + from_muzzle[1] * from_muzzle[1]);
		}
	...
	if (start[2] - target[2] < 160 && start[2] - target[2] > -16)
	{
		vec3_t dist;
		
		trace_t tr = gi.trace(start, vec3_origin, vec3_origin, aim_point, self, MASK_SOLID);
		if (tr.fraction < 1.0 && (!self->enemy || tr.ent != self->enemy))
		{
			...
			x = VectorLength(dist);
			t = x / vx;
			drop = 0.5f * sv_gravity->value * t * (t + FRAMETIME);
			tr.endpos[2] -= drop;
