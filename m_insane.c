...
void insane_walk(edict_t *self)
{
	if (self->spawnflags & 16 && self->s.frame == FRAME_cr_pain10) // Hold Ground?
	{
...
void insane_checkdown(edict_t *self)
{
	...
	if (random() < 0.3f)
		self->monsterinfo.currentmove = (random() < 0.5f ? &insane_move_uptodown : &insane_move_jumpdown);
}
...
void insane_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if (self->spawnflags & 8)
		insane_dead(self);
	else if ( (self->s.frame >= FRAME_crawl1 && self->s.frame <= FRAME_crawl9) || (self->s.frame >= FRAME_stand99 && self->s.frame <= FRAME_stand160) )		
